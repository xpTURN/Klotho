...
> **Dev-diagnostic build symbols (cross-engine note)**: the core gates many diagnostics — including the `EventDispatchWarnMs` warning — behind `#if DEBUG || UNITY_EDITOR` (some behind `#if DEBUG` alone). On **Unity** they are active in the Editor and in development players automatically: every development build defines `DEBUG`. On **Godot (.NET)** the core ships as a prebuilt Release DLL, so these diagnostics are **compiled out in every configuration**; only `KDebug`/`KTrace` calls in your own game code follow your project's defines — `Debug` and `ExportDebug` already define `DEBUG`, so add it to `ExportRelease` only if you want them there. The **.NET dedicated server** builds the core through the `Server~` project references, which do not see your game project's `<DefineConstants>`: build with `-c Debug` to compile the diagnostics in. Avoid `-p:DefineConstants=…` on the command line — it **replaces** `DEBUG`, `TRACE` and each project's own defines instead of adding to them.
