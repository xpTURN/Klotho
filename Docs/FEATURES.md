...
- **KlothoSessionFlow / KlothoFlowSetup** — recommended session construction layer
  - Mode-dispatched entry points: `StartHost` / `StartHostAndListen` (P2P host) · `StartLocal(simCfg, sessionCfg, roomName?)` (single player — one call, no socket) · `JoinAsync(strategy, transport, host, port, roomId, sessionCfg, ct)` — the unified guest entry, with `JoinP2PAsync` / `JoinServerDrivenAsync` convenience overloads that delegate to it · `ReconnectAsync` · `SpectateAsync` · `StartReplayFromFile`. Multi-mode games call `JoinAsync` with `KlothoModeStrategy.Resolve(simCfg)` and never branch on mode at the join site
  - **StartLocal** — single-player entry: folds session create + `HostGame` + auto-PlayerConfig + `SetReady` into one call, because the failure modes of assembling them by hand are silent (`SetReady` before `HostGame` returns without a log; a `MinPlayers` other than 1 leaves the start gate false forever). Copies the passed `ISessionConfig` and forces `MinPlayers = 1` on the copy, so a shared inspector `ScriptableObject` is never written to. Pairs with **`NullTransport`** — a socket-less `INetworkTransport` whose `Listen`/`Connect` return false on purpose — so a solo session binds no port and raises no firewall prompt. Fires `SessionEntryKind.Local`
  - **Replay recording is opt-out** — `KlothoFlowSetupBuilder.WithoutReplayRecording()` (default on). Per-peer and never on the wire; `Build()` flags it when combined with `WithReplaySave`
...
- **View layer**
  - **EntityView / EntityViewComponent** — entity-view base class and view-component interface (`EntityViewComponent` is *Unity-only*; Godot's counterpart is `EntityViewNode`)
    - **Standard transform pipeline** — `EntityView` performs lerp + `ApplyTransform` + `UpdatePositionParameter` populate in `InternalLateUpdateView` (fused with `_errorVisual.Tick`), so tick-rate < frame-rate environments reflect every per-frame `PredictedAlpha` change in the transform without stale-lerp stutter. The whole error-visual stage sits behind `EnableErrorCorrection`, so a game that leaves the flag off runs neither the delta lookups nor the smoother; `UpdatePositionParameter` zeros `ErrorVisualVector` / `ErrorVisualQuaternion` when the flag is off and when `EnableSnapshotInterpolation` is set (the verified-frame interpolation path already renders the corrected state, so applying the offset would double-correct). The smoother is ticked before the parameter is read, so the offset that hides a rollback lands on the frame the delta arrived on. Games override `OnUpdateView` / `OnLateUpdateView` for game-data cache + visual feedback; the transform pipeline itself is base-delegated
  ...
  - **EntityViewFactory / IEntityViewPool / DefaultEntityViewPool** — view creation / pooling. The base `EntityViewFactory` resolves `BindBehaviour` / `ViewFlags` from a 5-flag decision (rolls up `RequiresBindBehaviour`, `HasViewComponentInterpolation`, `RequiresErrorCorrection`, `RequiresSnapshotInterpolation`, `RequiresViewComponentBinding`) — games override only when a sample-specific override is required. `BindBehaviour` is assigned whole (the factory's answer covers the enum), while `ViewFlags` is **merged** through `ComposeViewFlags`: only the bits in the virtual `FactoryOwnedFlags` (`EnableSnapshotInterpolation` by default) come from the factory, the rest stay as the prefab authored them. A factory that decides further bits widens the mask by overriding the property. `CreateAsync` spawns at the entity's pose, so a pooled view never reappears at its previous tenant's position
  - **EntityViewUpdater** — simulation state → view sync; `Initialize(engine, factory = null, pool = null)` accepts the factory and pool from code as well as the Inspector (`null` = keep what is set). Owns the built-in **PlayerViewRegistry\<TView\>** (lifted from sample). A snapshot-interpolated view is kept until the render clock reaches the tick its entity left on (`OnUpdateView` stops at the entity's disappearance; per-frame interpolation continues through that grace), while a CSP view is destroyed immediately — its lifetime tick and its render tick are the same. EVU drives `Register` / `Unregister` automatically from `OwnerComponent` add/remove; game code uses `Get(playerId)` for lookup and subscribes to `OnViewRegistered` / `OnLocalViewRegistered` / `OnLocalViewUnregistered` for player-view event hooks
  ...
  - **VerifiedFrameInterpolator** — interpolation based on verified frames. `GetSnapshotPose(entity, engine) → SnapshotPose` answers pose, window-occupancy, the `alpha = 0` base pose and teleport-in-window in one query (`InterpolatePosition` / `InterpolateRotation` remain as wrappers). Both endpoints are version-checked and verified-boundary-checked, a teleport between them is held rather than lerped, and an entity the window has not reached yet holds the oldest Verified pose it will show instead of tracking prediction
  - **BindBehaviour / ViewFlags** — view-binding enums (Verified / NonVerified; snapshot-interpolation flags). Present on both engines (Godot defines them in `ViewEnums.cs`). `EnableSnapshotInterpolation` is factory-owned — setting it on a prefab has no effect, since mode and ownership decide it
