### New Rules

Rule ID | Category | Severity | Notes
--------|----------|----------|-------
KLSG_ECS001 | KlothoGenerator.ECS | Error | Duplicate Component Type ID
KLSG_ECS002 | KlothoGenerator.ECS | Error | Missing IComponent
KLSG_ECS003 | KlothoGenerator.ECS | Error | Component not unmanaged
KLSG_ECS004 | KlothoGenerator.ECS | Warning | Oversized component
KLSG_ECS005 | KlothoGenerator.ECS | Error | Missing partial keyword
KLSG_ECS006 | KlothoGenerator.ECS | Warning | MaxCount ignored on singleton component
KLSG_ECS007 | KlothoGenerator.ECS | Warning | [KlothoCleanup] on a [KlothoCoreComponent]
KLSG_ECS008 | KlothoGenerator.ECS | Error | [KlothoCleanup(DestroyEntity)] on a [KlothoSingletonComponent]
KLSG_ECS009 | KlothoGenerator.ECS | Error | [KlothoCleanup] argument is not a defined CleanupMode member
KLSG_DA001 | KlothoGenerator.DataAsset | Error | Missing partial keyword
KLSG_DA002 | KlothoGenerator.DataAsset | Error | Missing IDataAsset
KLSG_DA003 | KlothoGenerator.DataAsset | Error | Missing int constructor
KLSG_DA004 | KlothoGenerator.DataAsset | Warning | Ambiguous constructor
KLSG_DA005 | KlothoGenerator.DataAsset | Error | Duplicate DataAsset TypeId
KLSG_DA006 | KlothoGenerator.DataAsset | Error | Mixed user/generated DataAsset members (ctor(int) and AssetId property must be authored together)
KLOTHO_STRUCT_LAYOUT_MISSING | KlothoGenerator.ECS | Error | Missing [StructLayout(Sequential, Pack=4)] on KlothoComponent
KLOTHO_INTPTR_FORBIDDEN | KlothoGenerator.ECS | Error | IntPtr/UIntPtr field forbidden in KlothoComponent (platform-variable size)
KLOTHO_EMPTY_STRUCT_SIZE | KlothoGenerator.ECS | Error | Empty KlothoComponent tag struct requires Size=1 (Mono DivideByZero guard)
KLOTHO_ENUM_UNDERLYING | KlothoGenerator.ECS | Warning | Enum field without explicit underlying type
KLOTHO_BOOL_FIELD | KlothoGenerator.ECS | Warning | bool field in KlothoComponent (cross-platform verification)
KLOTHO_CHAR_FIELD | KlothoGenerator.ECS | Warning | char field in KlothoComponent (2B Managed vs Native)
KLOTHO_HFSM_HOST_FIRST_FIELD | KlothoGenerator.ECS | Error | IHFSMHost component's first field must be HFSMState (Unsafe.As offset-0 reinterpret)
KLOTHO_DET002 | KlothoGenerator.Determinism | Warning | Floating-point type in deterministic context
KLOTHO_DET003 | KlothoGenerator.Determinism | Warning | Non-deterministic API or type in deterministic context
KLOTHO_DET004 | KlothoGenerator.Determinism | Warning | Wall-clock time in deterministic context
KLSG_SS001 | KlothoGenerator.SerializableStruct | Error | [KlothoSerializableStruct] missing partial keyword
KLSG_SS002 | KlothoGenerator.SerializableStruct | Error | [KlothoSerializableStruct] not unmanaged
KLSG_SS003 | KlothoGenerator.SerializableStruct | Error | [KlothoSerializableStruct] missing [StructLayout(Sequential, Pack=4)]
