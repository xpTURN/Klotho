...
namespace xpTURN.Klotho.Deterministic.Navigation
{
    ...
    public static class FPNavMeshBuildPipeline
    {
        ...
        /// <param name="log">Optional diagnostic sink (engine console). Null = silent.</param>
            ...
            double bakeAgentHeight = 0, double bakeAgentClimb = 0)
        {
            ...
            SplitTJunctions(vertices, ref indices, ref areas,
                T_JUNCTION_EPSILON, T_JUNCTION_HEIGHT_EPSILON, log, logError);
            ...
            Action<string> log, Action<string> logError)
        {
            ...
            for (int iter = 0; iter < maxIterations; iter++)
            {
                ...
                {
                    logError?.Invoke($"[FPNavMeshBuildPipeline] T-Junction split aborted: triangle count " +
                ...
                log?.Invoke($"[FPNavMeshBuildPipeline] T-Junction split: iteration {iter + 1}, " +
                ...
                {
                    logError?.Invoke($"[FPNavMeshBuildPipeline] T-Junction split did not converge " +
