    ...
    public class FPNavMeshQuery
    {
            ...
            out FPVector3 hitPoint, out int triIdx)
        {
            ...
            if (!IsInsideGrid(originXZ, gridOriginX, gridOriginZ, gridMaxX, gridMaxZ))
            {
                ...
                {
                    // _logger?.KTrace($"[Raycast] RayAABB2D miss — ray does not intersect grid XZ bounds");
                    return false;
                }
                // _logger?.KTrace($"[Raycast] origin outside grid. tStart={tStart}");
            }
            else
            {
                // _logger?.KTrace($"[Raycast] origin inside grid. tStart=0");
            }
            ...
            while (_navMesh.IsCellValid(col, row))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        // _logger?.KTrace($"[Raycast]   tri[{tIdx}] skip (visited)");
                        continue;
                    }
                ...
                if (bestTri >= 0)
                {
                    ...
                    if (confirm)
                    {
                        ...
                        triIdx = bestTri;
                        // _logger?.KTrace($"[Raycast] HIT tri={triIdx} hitPoint={hitPoint}");
                        return true;
                    }
                }
                else
                {
                    // _logger?.KDebug($"[Raycast]   no hit in cell ({col},{row})");
                }
            ...
            // _logger?.KDebug($"[Raycast] MISS — grid exhausted");
            return false;
        }
