...
namespace xpTURN.Klotho.Deterministic.Physics
{
    ...
    public struct FPPhysicsWorld
    {
        ...
        public long GetStaticFingerprint()
        {
            ...
            h = FPHash.Hash(h, _staticColliderCount);
            for (int i = 0; i < _staticColliderCount; i++)
            {
                ref var c = ref _staticColliders[i];
                h = FPHash.Hash(h, c.id);
                h = FPHash.Hash(h, c.isTrigger);
