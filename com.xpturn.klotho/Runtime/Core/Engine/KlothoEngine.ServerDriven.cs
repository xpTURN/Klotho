    ...
    public partial class KlothoEngine
    {
        ...
        private void UpdateServerTick(float deltaTime)
        {
            ...
            if (_accumulator > maxAccumulator)
            {
                ...
                _logger?.KWarning($"[KlothoEngine][SD] ServerTick: Accumulator clamped: {dropped:F1}ms dropped ({dropped / _simConfig.TickIntervalMs:F1} ticks skipped)");
            }
        ...
        private void ExecuteServerTick()
        {
            ...
            if (xpTURN.Klotho.Diagnostics.FaultInjection.ServerGcPauseAtTick == CurrentTick)
            {
                ...
                if (pauseMs > 0)
                {
                    _logger?.KWarning($"[FaultInjection][SD] Server GC pause: tick={CurrentTick}, sleep={pauseMs}ms");
