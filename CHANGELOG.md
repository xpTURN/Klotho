...
- **What it is.** On a big map, one A\* search from a unit to a far destination is expensive and can
  give up before it finds anything. Planning in legs cuts the walkable surface into **nodes**, picks
  a route through those nodes first, and then asks the real A\* for **only the next leg** — the hop
  to the next boundary. Each search stays small no matter how far away the destination is.

- **What it buys, measured.** 800 agents on a 96×96-cell field, all ordered across the map at once:

  | | before | after |
  | --- | --- | --- |
  | the tick that takes the order | 1001.7 ms | **38.0 ms** |
  | units that got no path at all | 358 of 800 | **0** |
  | searches that hit the A\* budget | 3,226 | **0** |
  | paths clipped by the corridor buffer | 4,774 | **0** |

  The two caps stopped binding rather than being raised: a leg never leaves its node, so it cannot
  outgrow either one.

- **What it costs: a slightly longer walk.** A node-by-node route is not the shortest one. A unit
  crossing the field walks **1.10× the straight-line distance at worst**, and **1.01×** on a long
  crossing — the detour is roughly constant, so it matters less the further you go. Units do not
  stall or circle when they change legs.

  Two rules keep that number small. The route is priced from the unit's **actual position** and the
  **actual destination** at the two ends, instead of from the centres of the nodes they stand in;
  and a leg **aims along the boundary segment** at the point that makes the crossing straightest,
  rather than at its midpoint. Two more rules govern how a unit behaves at a boundary rather than
  how far it walks: a leg is sent on to the **next** crossing when it is handed the one just made
  (reaching a boundary leaves the unit on the near side of it), and a leg hands off at the unit's
  turning radius `v² / a` rather than at the tight arrival threshold used for a destination — no
  steering can hold a tighter arc, so asking a unit to pass within centimetres of a boundary it must
  turn at makes it orbit that point.

- **The engine turns it on itself, and the cell size is chosen for you.** `FPNavAgentSystem`'s
  constructor looks at the mesh it is built on and, when a flat search can run out of budget on it
  (`triangles > FPNavTuning.MaxIterations`), derives an abstract graph before the first tick and
  before Ready compares fingerprints. `FPNavTuning.AutoInstallAbstractGraph` is the switch, on by
  default. It asks only about budget exhaustion: a corridor clamped by the cap is walked and
  re-planned, not failed, and asking about that too would have put a graph on every mesh past 128
  triangles. A mesh within the budget gets nothing — every shipped Brawler stage.

  Node width depends on cell size *and* on how finely the mesh is triangulated, and that density
  varies **tenfold** across the shipped assets, so no fixed number is safe everywhere. The
  constructor starts at `GridCellSize × 16` and halves until the widest node fits the corridor cap,
  taking the first size that does. Deriving is *cheaper* at larger cells (8.5 ms at cell 64 against
  40.8 ms at cell 4 on the 22k-triangle asset), so it tries the cheap sizes first — on that asset it
  settles in two attempts at cell 32, 81 nodes, in about 20 ms (Release). A mesh no cell size fits
  is left flat with a warning.

  The same ladder is public as `FPNavAgentSystem.TryInstallAbstractGraphIfBeneficial`, for a game
  that turned the automatic install off and wants to decide later, or wants the either-condition
  contract (budget *or* cap; `exhaustionOnly` selects the constructor's). It never throws — it runs
  on your initialization path, where an exception is a failed boot — and every outcome is a value
  **and** a log line, including the ones where it decides to do nothing. A game that wires its own
  graph (cell size, cost fold or mask) turns the automatic one off in the tuning, or simply installs
  over it — `SetAbstractGraph` replaces, and says once what it discarded.

  `MaxIterations` is thereby two things: the search budget and the legs threshold. Lowering it turns
  legs on for more meshes; raising it to 65536 turns automatic legs off for a 22k-triangle stage
  without saying so. There is deliberately no separate threshold: the budget is the exact condition
  under which a flat search can fail, and a second number would drift from it.

- **Off is exact, and the fingerprint says which side you are on.** With
  `autoInstallAbstractGraph: false` and no graph installed, the agent system plans exactly the flat
  path it always did, bit for bit — that is how the two are compared on one fixture. Install one and
  where units walk changes, so the graph's checksum folds into `GetNavFingerprint`: on `Field` the
  constructor moves the fingerprint to the value the explicit helper produced before, which proves
  the two build the same graph. A mesh small enough not to need legs is left alone and its
  fingerprint does not move.

  The same fingerprint catches peers that disagree — legs on against legs off, or two different cell
  sizes — at the Ready exchange, before a match can diverge. Agents whose area mask is not the one
  the graph was built for quietly keep the flat path (the graph would otherwise promise them
  crossings that mask forbids); `DebugMaskFallbackCount` counts them.

- **A rebaking game pays nothing extra on the tick.** If your game rebakes the navmesh at runtime,
  every swap has to rebuild the graph — 10.2 ms on the 22k-triangle asset, inside the very tick that
  sliced rebaking works to keep short. It no longer happens there. The engine builds the graph on
  the frame *before* the swap, from the mesh the rebake driver is already holding, and the swap just
  adopts it.

  **You write nothing.** Register a navigation system that implements `INavGraphPreparer` —
  `FPNavAgentSystem` does — and the same frame heartbeat that paces rebake slices does this too.
  Missing it would be invisible (nothing breaks; a tick merely costs more), which is exactly why it
  is not left to a game to remember. A preparation is adopted only when it was built for that exact
  mesh **and** the mesh's contents still match, so a pooled and rewritten mesh can never be paired
  with a stale graph. When it does not land — the driver served the mesh from its cache, or rebuilt
  it synchronously — the swap rebuilds as before, and warns once, pointing at
  `PrepareAbstractGraphFor` for a game that swaps meshes by a path of its own.
  `FPNavMeshRebakeDriver.PeekPreparedMesh` is the read that makes it possible, and it moves nothing.

- **Numbers you can read to see whether any of this is working.** All are peer-local, outside the
  state hash, and never reset.

  | on `FPNavAgentSystem` | what it tells you |
  | --- | --- |
  | `DebugExhaustedWithoutLegsCount` | searches that ran out of A\* budget **with nothing shortening them**. Not the same as "no graph installed": a graph can be present and still not shorten a particular search, and those are the ones most easily missed. One warning per system accompanies it, and names why there is no graph — the switch is off, or no cell size fit |
  | `DebugLegEndedOnPlanTickCount` | legs that ended on the very tick they were planned, meaning the unit never travelled. Read as a rate against `DebugLegAdvanceCount`: near zero is healthy, climbing together means the turning radius `v² / a` has grown wider than a node and the unit re-plans every tick. A warning says so once, with both numbers |
  | `DebugGraphRederiveCount` | swaps that rebuilt the graph on the tick. Zero for a game with no graph — which is why this cost went unnoticed |
  | `DebugGraphPreparedAdoptedCount` / `DebugGraphPreparedMissedCount` | swaps that took a prepared graph, against swaps where one existed but did not fit |
  | `DebugMaskFallbackCount` · `DebugAbstractSearchFailedCount` · `DebugLegAdvanceCount` | mask fallbacks, node routes that do not exist, and legs completed |

  `FPNavMeshPathfinder.DebugLastSearchIterations` reports what the most recent `FindPath` **call**
  spent, and **0 when that call did not search at all** (an endpoint off the mesh, a blocked or
  masked endpoint, start and goal in one triangle). It belongs to a call, not to an agent: one
  pathfinder serves every agent in a tick.

- **Both editor visualizers show it.** Loading a mesh that either cap can bind on gets a notice
  saying which one, worded as *can* rather than *will*. The graph the constructor installed is shown
  and marked *automatic*; you can also derive one at a chosen cell size and cost fold, see its node
  and edge counts and its checksum, draw the node partition over the mesh, and watch legs replan as
  units move. Clearing it, or loading with the *auto legs* switch off, is how you look at legs
  **off** — which is how the plan-tick failure above was found.

- **The public surface is deliberately small.** `FPNavAbstractGraph` is a handle: build it, hand it
  to `SetAbstractGraph`, read it back from `FPNavAgentSystem.AbstractGraph`, and read `NodeCount`,
  `EdgeCount`, `MaxNodeDiameter`, `MaxLegCorridorTriangles`, `NodeComponentCount` and `Checksum` to
  tune it. One accessor is public, `NodeOf(triangleIndex)` — which node claims a triangle, `-1` when
  none does — and that is the whole partition, since an edge between nodes is exactly "two
  neighbouring triangles in different nodes". The tools draw the partition from that one call. Edge
  **costs** stay internal: publishing a cost would invite a second copy of a formula that can drift.

  Two of those numbers are easy to confuse, so they are named apart. `MaxNodeDiameter` counts
  **hops** across the widest node; `MaxLegCorridorTriangles` counts **triangles**, which is what the
  corridor cap counts, and is the one to compare against `FPNavTuning.CorridorCap`.
  `SetAbstractGraph` refuses a graph built from a different mesh, and one whose widest node cannot
  fit the corridor — that refusal is what turns "a leg fits" from a measurement into a guarantee.

  See [`Navigation.md` § Planning in legs](Docs/Navigation.md#planning-in-legs) and
  [`Navigation.Rebake.md`](Docs/Navigation.Rebake.md) for the rebake side.

- **When a search still runs out of budget, it hands back a partial path.** A mesh whose nodes never
  fit the corridor cap, or a game that turned legs off, still has searches that exhaust
  `MaxIterations` — and until now those returned nothing: the unit sat at `PathFailed` until the game
  gave it a new order. With `FPNavTuning.PartialPathOnExhaustion`, on by default, such a search
  returns the corridor to the **closest point it reached**, the unit walks it, and plans again from
  the end — the way Detour and Unity's NavMesh answer the same situation.
  `new FPNavTuning(partialPathOnExhaustion: false)` is the pre-0.13 answer, bit for bit: the off
  fingerprints and a corridor golden over `Field` are pinned alongside the default ones.

  Measured on the shipped `Field` mesh (22k triangles) with legs off, 256 orders, 105 of which
  exhaust the default budget: **93 of 105 arrive**, median one hop, no cycles. On a serpentine every
  clipped corridor end lies *further* from the goal than the hop began, and the unit still arrives —
  progress is judged at the closest node, not at the point walked to.

  What it costs, and what a game should look at: each hop is a full-budget search (bounded per hop by
  the fixes below, but a long order is now several searches rather than one), and the new way to fail
  is *after moving* — a pocket that faces the goal looks like progress, the unit walks in, and fails
  there, so any code that treats `PathFailed` as "the unit did not move" wants a look (Brawler's bot
  fallback is gated on standing inside a building and is unaffected). A partial is only given when it
  brings the unit closer by more than its own hand-off radius (`v² / a`), so a re-plan never starts
  where the last one did. The switch joins the navigation fingerprint as its own term (zero when
  off), so peers that disagree are refused at Ready, FullState and replay load.
  `FPNavPathFailure.Diagnose` given a pathfinder now tells `NoRoute` from `BudgetExhausted`; both
  visualizers label a partial Find Path result; `DebugPartialPathCount`,
  `DebugPartialRejectedCount` and `DebugPartialHandoffCount` count partials given, refused for no
  progress, and walked to their end.

- **The failure diagnosis is split into its two halves, and the editor simulators search on a
  pathfinder of their own.** `FPNavPathFailure.DiagnoseEndpoints` is the cheap half (footing,
  endpoint, swap marker) and `FPNavPathFailure.SearchVerdict` the expensive one (the re-search that
  tells `NoRoute` from `BudgetExhausted`); `Diagnose` is the two composed. Handing `Diagnose` the
  pathfinder the agent system runs on — the very instance it says must never be given — re-runs a
  full-budget search on the engine's stack at every repaint of a `PathFailed` agent and pushes its
  *budget exhausted* counter up with no agent involved. Both simulators search once per failure on a
  separate pathfinder built with the engine's tuning and remember the verdict, re-asking only the
  endpoint half each repaint. Removing an agent slides its budget attribution and verdict down with
  it.

- **A clipped partial corridor ends at its farthest point from the unit, not at the cap.** The best
  node can never sit inside the unit's reach radius (its progress is at least that radius, and
  progress cannot exceed distance), but the corridor cap cuts at a triangle count, and on a
  switchback that cut landed straight across the wall from the unit — inside the radius at full
  speed. The hand-off then fired on the tick the plan was made, cleared the repath cooldown, and the
  next tick planned again: a full-budget search per tick until the unit's own motion carried the end
  away. Measured on a 28-cell serpentine at speed 7, 62 such plans in bursts of up to 17 ticks
  (96 hand-offs where 31 suffice); on a 96-cell one at speed 9, 191 hand-offs against 71 at speed 7.
  A clipped chain is cut at the kept prefix's farthest triangle — a valid prefix, minus exactly the
  part that doubled back, which on a switchback is the turn — and both fixtures drop to 31 and 88
  hand-offs with no plan-tick hand-off at all. Unclipped partials, the progress rule and the goldens
  are unchanged; off stays bit-identical. `DebugPartialEndedOnPlanTickCount` on the agent system
  counts the event, exact without a graph like the hand-off counter.

- **A goal already in the open set when the budget runs out comes back as a whole path (with partial
  paths on), and the exhaustion warning says what the unit actually got.** The budget can run out in
  the window between the push that discovers the goal and the pop that would finish — about ten pops
  on the shipped `Field` asset, one on a switchback, and never once in 1,045 exhausted searches there,
  so this is rare. In that window the best-node rule would hand back a partial to the triangle
  *beside* the goal (the goal's heuristic is measured at its entry edge, and a neighbour's edge
  midpoint is usually nearer the goal point), and the unit would spend one more hop and one more
  full-budget search to finish. The chain behind a queued goal is a real path, so it is handed back
  whole; the exhaustion stays counted. With the switch off the answer is still `false` — that search
  failed before too, and a path there would move the frame hash without a revision bump. The
  once-per-system warning has three outcomes instead of two: reading the pathfinder's last-call flag
  described only the second of two searches, and said "no path" when a flat retry after an exhausted
  leg search — or this recovery — had produced a whole one.

- **The editors draw a partial path to where it actually ends, and the play-mode overlay reconstructs
  routes with the game's tuning.** A Find Path that funnels a partial corridor toward the goal draws a
  polyline from the corridor's last triangle straight across ground the search never reached — the
  funnel appends its end point as the last portal unconditionally — under a label saying the path
  stops short. Both visualizers call the eight-argument `FindPath` and funnel toward its `partialEnd`,
  the same point the agents steer at. The Unity scene overlay that redraws a live agent's route in
  play mode takes the tuning off the game's query instead of building a default-tuned pathfinder over
  it, so a game with a different budget sees its own corridor rather than a default one — or the "no
  path" straight line the default budget produced. `DebugLastPathWasPartial` on the pathfinder says
  whether the last call was clipped short; the partial end is the overload's `partialEnd`.

- **The editors' failure sentence follows the exact per-agent verdict, and the re-search asks for
  the engine's minimum progress.** A failure has two attributions: the re-search's (`NoRoute` or
  `BudgetExhausted`, exact per agent, with or without a graph) and the tick-wide one (`FailureKind`,
  exact only without a graph). The sentence is keyed on the verdict first, so the prescription —
  *planning in legs makes each search local, which is what removes this* — appears exactly when the
  exact verdict says the budget ran out, and the tick-wide attribution is the fallback for the cases
  the re-search cannot answer. `FPNavPathFailure.Describe(BudgetExhausted)` is mode-neutral — *the
  search budget ran out before it could decide* — because whether partial paths were on is the
  caller's to know; the editors add that clause themselves from their agent system's tuning.
  `Diagnose` and `SearchVerdict` have overloads taking the agent system's `WaypointThreshold`, so a
  system that changed it (and whose agent's `Speed² / Acceleration` is below it) gets a re-search that
  refuses the same partial the engine refused, instead of accepting it and answering "cannot be
  explained". The last search that fails an agent is always the flat one to its destination — a
  failed leg search falls back to it — so the re-search's target is already right.

- **Both NavMesh visualizers load a mesh with their own tuning.** Three inputs next to Load —
  `Search Budget` (`FPNavTuning.MaxIterations`, minimum 1), `partial paths`
  (`PartialPathOnExhaustion`) and `auto legs` (`AutoInstallAbstractGraph`) — feed
  `LoadFromBytes(bytes, tuning)`, and the simulator, its diagnosis pathfinder and the load-time
  exhaustion warning pick the tuning up from there. Without them no shipped stage could be made to
  exhaust a search, so the PARTIAL labelling, the partial-path drawing and the budget diagnosis were
  unreachable in the editor; and since a lowered budget is exactly what turns the automatic graph on,
  the third switch is what lets an exhaustion be shown at all. Applying is reloading: the query,
  pathfinder, funnel and agent system hold the tuning they were built with and the agent system
  refuses a trio that disagrees, so the section says when the input differs from what is loaded and
  offers the reload, which clears agents, placements and the leg graph like any load. The corridor
  cap is deliberately not a knob — the agent component's corridor buffer is sized to it at compile
  time.

### Changed — the default navigation fingerprint moves, and the crowd-scaling guidance says what spreading the load cannot do

- **Every game that never named a tuning has its navigation fingerprint move, and that is a
  deliberate compatibility break.** Two defaults are new: `PartialPathOnExhaustion` is its own term
  of the fingerprint, so the default stack's fingerprint moves by exactly that term on every mesh;
  `AutoInstallAbstractGraph` folds the graph's checksum in on a mesh past the budget (`Field`, not any
  shipped Brawler stage). Replays recorded before 0.13 are refused by a 0.13 build, and a 0.13 peer
  and a 0.12 peer refuse each other at Ready and on FullState. That refusal is the protection
  working; keeping the fingerprint still would have let an off peer and an on peer meet and diverge
  silently. The two breaks ship in one release on purpose — one break, not two — and a game that
  wants its 0.12 replays must name **both** switches off: `new FPNavTuning(partialPathOnExhaustion:
  false, autoInstallAbstractGraph: false)`, or the replays are refused by the graph instead. The four
  pinned fingerprints and the corridor goldens in the test suite carry both values, so the off state
  stays proven identical to the old one. Both editors' tuning input defaults to on as well.

- **[`Navigation.md` § Crowd Scaling](Docs/Navigation.md#crowd-scaling)** described two levers — a
  bounded admission queue, and splitting the agent array into clusters — without saying what they
  cannot do. Both **spread** work; neither makes a search cheaper. So neither helps the ~42% of
  searches that run out of A\* budget and come back with no path: spread over ten ticks, those
  become ten ticks of failures instead of one. The queue also does not fit a 10 Hz tick — by the
  last admission tick the units admitted earlier are already moving, so the steady-state cost runs
  alongside it (~113 ms against a 100 ms budget, before any game logic).

- **Raising `MaxIterations` is now called out as the wrong direction**, which matters because
  `FPNavTuning` made it reachable. The searches that exhaust the budget are the expensive ones, so a
  bigger budget turns fast failures into slow successes and raises the total. A long order on a big
  map needs a cheaper *kind* of search — which is what planning in legs is; and since 0.13 a bigger
  budget also turns automatic legs off.
...
- **`KlothoSessionFlow.StartLocal(simCfg, sessionCfg, roomName?)` starts a one-player session in one call.** A solo match already worked as a P2P host with nobody in it, but assembling it meant `StartHost` + `HostGame` + `SetReady` in that exact order with `MinPlayers = 1` remembered in the inspector — and both ways of getting it wrong are silent: `SetReady` before `HostGame` returns without a log, and any other `MinPlayers` leaves the start gate false forever. `StartLocal` folds the sequence, including the auto-`PlayerConfig` send that the host path cannot do (it runs before `HostGame` assigns an identity; `StartLocal` owns the ordering and fires it after). The passed `ISessionConfig` is **copied** and `MinPlayers` forced to 1 on the copy — your object is never written to, which matters because Unity's `USessionConfig` is a `ScriptableObject` whose play-mode edits persist. Everything else stays as authored, including `MaxPlayers` (games derive bot id ranges from it) and `CountdownDurationMs` (the default 3000 means the returned session does nothing for three seconds — author `0` for an instant start).

- **`NullTransport` — an `INetworkTransport` with no socket**, for `StartLocal`. Every send is dropped and no event is ever raised, so a single-player build binds no port and triggers no firewall prompt. `Listen` and `Connect` return `false` on purpose: reporting success would tell a game that remote players can arrive, and `StartHostAndListen` already reads `false` as a bind failure and tears the session down.

- **`SessionEntryKind.Local`.** `IKlothoSessionObserver.OnSessionCreated` reports it for `StartLocal` sessions. The value is appended last, so existing ordinals are unchanged and existing code still compiles — but a `kind == SessionEntryKind.Host` branch will **not** see a local session. Games adopting `StartLocal` should extend those branches; the distinction is deliberate, since "a room others can join" and "alone for the whole match" need different UI and different result handling.

- **Replay recording is now opt-out: `KlothoFlowSetupBuilder.WithoutReplayRecording()`** (`KlothoFlowSetup.EnableReplayRecording`, default `true`). Per-peer and never on the wire — recording is a decision about this machine's memory, not something a host imposes on everyone. Use it for a long solo session that will never save a replay: the recorder's buffer grows with the match and is bounded only by its end. Turning it off does **not** disable the host's tick-0 bootstrap state or the pre-tick-0 divergence hash; both are taken regardless. `Build()` warns (and `Build(strict: true)` throws) when it is combined with `WithReplaySave`, since there would be nothing to save.

### Added — replay: the file now carries what it takes to re-run it

- **The replay file records what it was recorded against.** Four fingerprints (component-registry layout, static colliders, navmesh, and the game's own `IGameFingerprintSource` slot), the initial snapshot's hash **and the tick it was taken at**, a completion marker, and the two process-global layout inputs (`PrunedComponentTypeIds`, the per-component maxCount overrides). All are on `IReplayMetadata`. The point is *attribution*: when a re-simulation disagrees with the recording, these say whether the run was made by a different build or different content, which is the difference between "this player cheated" and "you are verifying it wrong". Nothing here is signed and every value is a non-cryptographic fold, so a forger rewrites them along with the payload — what they buy is the ability to dismiss an honest client's failure.

  The fingerprints are captured **at the same instant as the snapshot**, not at recording start: the navigation term moves with runtime rebakes, so any other moment would describe a world the recorded inputs never ran in. `0` in a fingerprint means "no such source registered" and is normal for a game with none.

  The two layout inputs are **recorded, not restored**. The component layout is frozen process-wide before any replay loads, so `ToSimulationConfig()` deliberately leaves them out rather than assigning values that could not take effect; a verifier reads them to *start* a matching process.

- **`IReplayMetadata.EndReason` (`ReplayEndReason`) says why a recording ended** — `Normal` for an ordinary stop (a dropped unconfirmed tail is still normal), `CorrectiveReset` / `ResyncRequest` for a desync-recovery truncation. A replay shorter than the match is honest, and this is how a reader tells that apart from a suspicious one instead of guessing from `TotalTicks`. `Unspecified` (`0`) means nobody stamped a reason and is a bug in whatever ended the recording — the value exists so that case is visible rather than indistinguishable from a clean end.

- **`IReplayMetadata.InitialRoster` records the participants the tick-0 world was built from, in creation order** (`IReplaySystem.SetInitialRoster`). The order is state-hash input — participant entities are created by walking the list — so late joiners must never be appended to it, which is why it is not called `ActivePlayerIds`. It is recorded **only by a peer that built tick 0 itself**, so an SD client's recording leaves it empty; that emptiness is the signal "this replay cannot be reconstructed" and there is no separate flag. `InitialRoster.Count` must equal `PlayerCount` when it is non-empty; any other disagreement is treated as a corrupted file.

- **A replay also records the per-player data the tick-0 world was built from** (`IReplayMetadata.InitialEntitlementData` + `InitialEntitlementLengths`, index-parallel to `InitialRoster`), and `PlayerJoinCommand` carries the joining player's copy so a late join reproduces too. Without this, playback read "nothing was issued" — the engine sources such values from the network service and a replay session has none — and the rebuilt world differed with **identical entity and component counts**, which reads as a determinism bug rather than a missing input. The failure was not one-directional: a diverged re-simulation whose result happened to match the recorded claim would have been reported *verified*.

- **`IKlothoEngine.GetPlayerEntitlement` answers from the recording when no network service exists.** Game code is unchanged — it keeps calling the same accessor, and the live path still wins: a network service that answers `null` has *answered* (that player was issued nothing), so the recorded table is consulted only when there is nobody to ask. Reversing that order would make a **live** match seed tick 0 as though nothing had been issued.

### Added — replay: tick 0 is rebuilt, and the state can be hashed at any tick

- **A verifier can rebuild tick 0 instead of trusting the recorded snapshot.** Re-simulation is authoritative about what the recorded *inputs* produce and says nothing about the state they start from — which was bytes the recording peer wrote, and forging them took no simulation at all. `KlothoEngine.StartReplay(data, ReplayInitialState.Reconstruct)` (threaded through `KlothoSessionFlow.StartReplay` / `StartReplayFromFile`) reconstructs the initial world from the recorded roster, seed and config through the same `BuildInitialWorld` a live match runs, and never reads the snapshot bytes — so substituting them achieves nothing. `RestoreSnapshot` remains the default and is what a viewer wants; reconstruction needs game callbacks (`OnInitializeWorld` runs) and is for verification.

  The engine reports both `ReplayTick0Reconstructed` and `ReplayTick0Hash`, and Brawler's `--verify` prints them as **`tick0=reconstructed|restored`** and **`tick0hash=ok|mismatch`** — two fields, not one, because the difference is load-bearing: a verified *restore* means only "these inputs produce this result", while a verified *rebuild* also means the starting point is this build's own. A rebuilt state that disagrees with the recorded hash is **unverifiable (`20`), never a cheat verdict** — its commonest cause is a content revision no anchor covers.

  **What is left open**: after the rebuild a forger no longer picks an arbitrary starting state, only the four inputs that produce one (roster, seed, stage, match config), all still the recording peer's. Closing that needs a server-issued seed and a signature, neither of which the format has.

- **`KlothoEngine.CurrentStateHash` folds the state the simulation holds right now, exactly the way `ReplayTick0Hash` does** — so the two are comparable, and so a value taken after playback covers every component a match result does not project. It is a **differential** signal, not a claim check: no end hash is recorded, so there is nothing in the file to check it against. What it buys is the case a result comparison is blind to — a re-simulation that diverged mid-match can still arrive at the same winner, and then a broken playback verifies green. Verification had a byte comparison at tick 0 and nothing at all over the thousands of ticks after it. On `KlothoEngine`, not on the interface.

- **Brawler's match config carries the room capacity** (`BrawlerMatchConfigData.MaxPlayers`), and every peer takes bot numbering from it instead of from its own `SessionConfig`. Bot ids are numbered past `maxPlayers` and their spawn slots follow from that id, which makes the capacity tick-0 state: with it living only in each peer's local config, a verifier rebuilding tick 0 had no way to know it — and a P2P guest whose local value differed would have built different bots. `0` means "not stamped" and consumers fall back to their local value (the dev lobby issues no capacity), while `--verify` refuses to rebuild without one (`20 matchConfig=noMaxPlayers`) rather than guessing: a guess builds a world with identical entity and component counts and a different hash, which reads as a determinism failure.

### Added — replay: `--verify` re-runs a recording in the server build (Brawler sample)

- **`dotnet run --project Samples/Brawler/Server -- --verify <replay.rply>` re-simulates a replay and compares the result it derives with the result the client claimed.** A mode of the existing dedicated server rather than a second executable, so the verifier and the room server compile from one set of game sources and read one set of baked assets — a verifier that resolved stage geometry through its own copy could disagree with the server about what stage 2 is, and only the replay's environment fingerprint would notice.

  **Exit codes are the verdict**: `0` verified, `10` claim mismatch (a cheat suspicion — and a *successful* run), `20` unverifiable, `30` unreadable file, `2` usage (an unknown option, or no path at all). `1`–`9` remain process failure, as everywhere else in that binary. The gap is deliberate: a crashed verifier must never read as a detected cheat. Each run also prints one machine-readable line naming *why*, and the process returns the worst verdict of the batch.

  **Unverifiable is not a pass.** A re-simulation always agrees with itself, so `Brawler` stamps what it claims happened into the replay's `GameCustomData` at match end (`BrawlerReplayClaim`) and the verifier compares against it. A replay with no claim, with absent (all-zero) anchors, with an anchor that disagrees, or one a desync recovery truncated, is reported unverifiable — never verified. The first two of those are things an attacker controls directly, which is why absence is a verdict rather than a shrug.

  **What it does not do**: the anchors are unsigned folds that a forger rewrites along with the payload, and there is no per-tick hash track — so a mismatch says *that* the runs diverged, not *where*. (The tick-0 state is not the client's bytes — see the section above.) See Docs/Replay.md §10.

- **The verdict line says how much of the environment it actually compared**: `anchors=ok:3/3`, `partial:n/3(names)` or `unchecked`. An environment term is compared only when *both* sides carry a value, and zero is a legitimate "not provided" — a game that registers no `IGameFingerprintSource` reports 0 there — so a count is the only honest form: "one term matched and two were never looked at" must not read as a full environment match. A file carrying **none** of the three environment terms is `unverifiable` rather than verified: there is nothing to attribute a divergence with, which is what that verdict means. Games that register none of the three sources are affected by design. `tick0=`, `tick0hash=` and `endhash=0x…` ride the same line — the last one on every line that gets that far, the claimless one included, since the line with nothing else to compare is exactly the one that needs it.

- **The verdict line also says *which match* the file belongs to.** `match=<id>` when the lobby issued one, and otherwise the reason it cannot be said: `unattributed` (the payload carries no identity — a solo, P2P or lobbyless-server match authors its own config; normal), `legacy-payload` (the game payload predates the identity field; old, not suspicious — told apart by payload *length*, since `BrawlerMatchConfig.Encode` is fixed-width and an older blob is simply shorter, whereas a current payload with no bots and no stamped values decodes to the same zeros an old one does). It deliberately does not report *"an identity was expected and is missing"*: that needs to know the match was lobby-issued, and nothing in the file says so — a lobby's payload and a lobbyless server's have the same fields and both stamp a capacity, so inferring an issuer would mark every lobbyless-server and P2P-host recording as suspicious. Whether an identity was *owed* is for whoever holds the issue record. Kept separate from the exit code on purpose — *"is this file honest"* and *"which match is it"* are different questions, and a file with no identity can be perfectly verified while proving nothing about a specific match. Without an identity nothing stops a player from running a one-shot attempt repeatedly and submitting only the best outcome; every one of those files is honest.

- **`BrawlerMatchConfigData` carries the lobby's match instance id** (`FixedString64`), and the dev lobby stamps it along with the room capacity. It rides the match config because that is what the core propagates to every peer *and spectator*, so it lands in every peer's replay metadata with **no replay-format change**. The replay's own `SessionId` is not usable for this: it is a local `Guid.NewGuid()`, different in every peer's recording. The lobby **refuses** to issue an id that does not fit the 62-byte budget rather than stamping a truncated one — two ids cut to the same value would merge two different matches.

- **`--save-replays <dir>` writes each room's replay when its recording stops** (off by default). The server records either way, so without this the buffer grows for the whole match (~114 bytes per tick per room) and is discarded — the one combination that pays the cost and gets nothing. The server's file is the only server-driven recording that can be *rebuilt* rather than restored (a client receives its initial state from the server and carries no tick-0 roster), and it is the authority's own record when a result is disputed. Files are named after the match identity, never a fixed name.

- **`--verify … --log=<level>` opens the simulation log for a diagnostic run.** The default stays `Warning`: a 5,000-tick playback at `Information` prints the game's whole per-tick chatter. Brawler's join-time loadout seed logs its masks there (`[Loadout] tick=… pid=… bytes=…`), which are otherwise invisible — they are seed state no line printed and the match result barely projects, and `bytes=0` on playback means "nothing answered, so this seeded *all owned*", which is not what a gated player had. The level is checked with `Enum.IsDefined` and not only `TryParse` (which accepts any *number*, so `--log=999` would parse into an undefined level and silence the run); a bad value warns and keeps the default rather than failing the batch, because a log level says nothing about whether the replays verify. Every token beginning with `--` is an option, never a path — a replay path never begins with `--`, and even on POSIX such a file has to be spelled `./--x.rply` — and an unrecognised one exits `2` naming the token, so a typo can never be reported as an unreadable file.

- **Docs/Replay.md §10** gains the comparison protocol: which record is authoritative per match kind (dedicated+lobby / dedicated lobbyless / P2P / solo), the attribution axis, and when to turn server-side recording on — or off.

### Fixed — replay: a replayed late joiner now enters the world

- **The join tick was not wired on the replay path.** A replay session is built through the callbacks overload of `KlothoEngine.Initialize`, and both join subscriptions (`OnPlayerJoinedNotification` → participant slot + the game's `OnPlayerJoinedWorld`, and `OnPlayerJoinedEntitlement` → the replay entitlement table) existed only in the *networked* overload. So playing back a match with a late join created no participant slot for the joiner, never ran the game's join-time seed, and read none of the per-player data the file carried for that player — while the recording itself was correct throughout. The subscriptions now also happen on the replay path (the spectator path, which builds its world from full states rather than a command stream, deliberately stays out).

  **Any replay of a match with a late join has been diverging since long before the per-player record existed** — the participant slot alone predates it by months. Nothing reported it: tick 0 was byte-compared and matched, the final result matched the claim, and the whole middle had no check to fail. In the match that exposed it, the replayed joiner's spawn landed on a recycled entity index (`14/v3`) where the live match had allocated a fresh one (`18/v1`) — a divergence of exactly the two entity creations that were missing — and the verdict still read `verified`.

- **Replays of late-join matches change trajectory with this fix, and the direction is diagnostic.** Playback now converges on what the live match did, so a verdict that moves should move from `10` (claim mismatch) toward `0`. A file that goes `0` → `10` after upgrading is evidence against the fix or against the file, not a normal consequence of it.

- **New gates**: the engine-side join wiring (`Samples/Klotho.Runtime.Tests`, including a scope guard that keeps the bare overload out of the join path) and the end of the chain into game state — a late joiner's loadout seed, checked in both directions, since an unread entitlement decodes to *everything owned* and would make a restricted player stronger rather than weaker (Brawler server `--test`).

### Changed — one SessionConfig per room

- **`SessionConfig.Clone()`** (public, memberwise — mirrors `SimulationConfig.Clone`). A caller that needs its own instance had only the internal `CopyOf`, which names every field because it converts from any `ISessionConfig`; a hand-copied list in a sample would silently drop the next field somebody adds.

- **The Brawler dedicated server now builds a SessionConfig per room** (`WithSessionConfig(matchCtx => …)`) instead of handing every room one shared instance. No value changes today — it removes the shared mutable object and opens the seam a per-match seed uses, since the factory receives the resolved `MatchConfigContext`.

### Fixed — physics contacts: the A→B normal contract, and a bounded snapshot copy

- **`sphere↔capsule`, `sphere↔mesh` and `capsule↔mesh` contacts returned the normal backwards**, so overlapping bodies were pushed *into* each other instead of apart, and a sphere or capsule resting on a **static mesh floor was pushed down through it**. Reproduced: two bodies 1.5 apart ended up 1.108 apart; a capsule penetrating a mesh floor by 0.15 sank a further 0.11 per step. The three pairs are corrected in `NarrowphaseDispatch` — the primitive tests in `CollisionTests` keep their own sign, and the dispatch's `Flipped()` corrections (10 call sites, `Test` and `TestMulti` alike) now cover every reversed pair rather than some of them. Static **box** colliders were always correct, and the bug could never surface as a desync — every peer computed the same wrong result. Reported by @snizkorod in [#7](https://github.com/xpTURN/Klotho/issues/7).

- **`NarrowphaseDispatch`'s typed shape overloads now use the same A→B convention as the `FPCollider` dispatch.** `Test(ref FPSphereShape, ref FPBoxShape)` and the sphere/capsule pair previously returned B→A, so the same logical query gave opposite normals depending on which overload you reached for. Only the collider path is used inside the engine; game code calling the typed overloads directly will see the sign change.

- **`SetSkipStaticGroundResponse`'s slope band is now defined on the static surface normal**, so it behaves the same for every collider type. It previously read the raw contact normal, which meant the "don't push a body standing on a walkable slope" rule fired **only for mesh colliders — and only because their normal was inverted**; the same 30° slope built from a box collider was never skipped. Measured: a moving sphere on a 30° slope now skips the response on both mesh and box (it previously skipped on mesh, and took a 0.299 push on box), while near-flat ground (≥ 0.95) keeps its response as before. **This changes behaviour** for games that use `SetSkipStaticGroundResponse(true)` with tilted box statics — the Brawler sample is one.

  Physics-driven scenarios therefore produce different state hashes than before. Replay files stay loadable (no layout change), but a recording made before this change will not re-play bit-identically.

- **`FPPhysicsWorld.CopyContactsTo` / `CopyStaticContactsTo` / `CopyTriggerPairsTo` now copy at most `buffer.Length` entries** and report what they actually copied. They previously wrote `list.Count` entries into a caller-supplied array with no bound, and the engine's own caller (`PhysicsSystem`) hands them fixed 256- and 64-entry buffers — so a body count large enough to produce more contacts than fit turned a debug view into an `IndexOutOfRangeException` in the middle of the tick. These three views feed the debug visualizers only, so the tail is dropped rather than grown: no simulation state is involved, and the cap is the caller's array length, which is identical on every peer. A `null` buffer now yields `0` instead of throwing. Reported in [#7](https://github.com/xpTURN/Klotho/issues/7).

### Fixed — a build no longer opens its log file where it cannot write

- **`KlothoLogger.CreateDefault` takes a `directory`, and when it is omitted the default is an absolute writable path per platform** — the project root in the Editor, `Application.persistentDataPath` in a Windows/iOS/Android build. The `RollingFile` sink's own default (`"Logs"`) is a *relative* path, so it resolves against the process working directory: the project root in the Editor, where nothing ever failed, but a read-only install folder (Windows) or a sandboxed bundle root (iOS/Android) in a build, where the directory cannot be created — and since the sink opens its file on first write, that surfaced as an exception thrown out of the **first log call**. Existing calls compile unchanged; their log files move to the new default root.

- **The Brawler sample resolves its replay path the same way** (`GetUserDataPath`), and passes that root to the logger so logs and replays land together on every platform. `Application.dataPath + "/../Replays"` only holds in the Editor: a build's `dataPath` is the install folder (Windows) or inside the `.app` bundle (iOS), so going one level up and writing there fails. The helper builds the path string only — directories are still created by whoever writes first (`ReplaySystem`, the rolling-file sink).

### Breaking changes

- **Replay files are no longer backward compatible, and the loader now says so.** `ReplayMetadata.CURRENT_VERSION` is **5** and the guard is `!=`, not `>`: a file from any other version is rejected outright instead of being read with the current layout. The old `>` guard let every earlier version through to be misparsed, and version `2` is skipped deliberately — it was issued briefly once before and files carrying it exist. Existing `.rply` files will not load; re-record them. An empty per-player record is valid — most matches have no issuer — so it is the version, not the field, that distinguishes an older file. The rejection reason now travels in `ReplayLoadException.Message` (it used to be buried in `InnerException`, so a game showing `e.Message` printed only "deserialize failed").

  Brawler's own match-config payload grew too (4 → 72 bytes: the room capacity, then the match identity), so a Brawler replay recorded before this release decodes as `legacy-payload` even where the replay layout would have read it. Re-record those as well.

- **`IReplaySystem.SetInitialStateSnapshot` takes the snapshot's tick**: `(byte[] snapshot, long hash, int tick)`. The hash previously reached only the `OnInitialStateSnapshotSet` event and never the file, and the tick did not exist at all — a snapshot hash that cannot say which tick it hashes is not an anchor. Only game code with a custom recorder calls this; the engine injects it on every ordinary path. `IReplayRecorder.StopRecording` gains an optional `ReplayEndReason` (defaulted, so existing calls compile unchanged).

- **`PlayerJoinCommand` gained a variable-length field** (`byte[] Entitlement`), so its serialized layout differs from earlier builds — peers and servers must run matching builds, as always. Commands are length-prefixed on the wire and in the replay buffer, so only `GetSerializedSize` had to follow. Game code constructing this command directly must **always assign the field, `null` included**: it is the command's first reference field and `CommandPool` does not clear fields, so a rented instance can still hold the previous joiner's bytes.

- **`ISessionConfig.RandomSeed` is now the authored request, not the last effective seed.** The documented contract — *0 is auto-replaced on the host*, therefore a non-zero value is used — was not implemented: `StartGame` drew a fresh `Environment.TickCount` on every match and discarded whatever was authored, on the P2P host and the dedicated server alike. The substitution has moved from session build time to match start, so a pinned seed now reaches the match, replay metadata included. Two consequences for game code:
  - **The config field is no longer written back with the effective seed.** It stays what you authored for the life of the session — commonly `0`. Code reading `engine.SessionConfig.RandomSeed` after game start used to get the seed in play and now gets the authored value; read `IKlothoEngine.RandomSeed` (or the network service's `RandomSeed`) instead. Guests are unaffected: their config was never authored, and late-join / reconnect / spectator / SD-client paths still carry the received effective value.
  - **A seed pinned on a `SessionConfig` shared across rooms makes every room run the same match.** `WithSessionConfig(value)` hands one instance to all rooms. `RoomManager` warns once when a room is created carrying a non-zero seed a live room already holds — compared by *value*, so a per-match factory that clones a pinned config is caught too, not just the shared instance. Use the per-match factory overload (or `SessionConfig.Clone`, new above) to give each room its own. Sequential reuse in a single-room server is silent.

  `0` keeps meaning "generate one", so that single value still cannot be pinned explicitly; every other `int` can, negatives included. No wire or replay-file format change — the seed travels in the existing `GameStartMessage.RandomSeed` field and replays recorded before this change play back identically.

### Tooling

- **`Tools/run-all-tests.sh` now runs the Brawler server's own `--test` suite.** That suite is the only cover over multi-room lifecycle, the replay verifier and game-side join seeding, and the runner did not know it existed — which is how a newly written join-seeding gate could sit outside the one command that claims to run everything. It follows the runner's existing design: each configuration is run and tee'd to `servertest-brawler-<cfg>.log`, and the summary parses only that file, one row per configuration (`--no-server-test` skips it, `SERVER_RC` folds into the exit code). Rows are deliberately not split per sub-suite — sub-suites keep being added and renamed, and an `expected` list enumerating them breaks every time. One pre-existing property now bites wider: the runner writes a fixed `Logs/test-results` and clears it on start, so it is **not safe to run twice concurrently**, and the server logs now share that directory.

### Docs

- **The ECS memory guide now says that reservation costs tick *time*, not only bytes.** `Frame.CopyFrom` is one `Buffer.BlockCopy` over the whole reserved heap, executed every tick by the history ring, so a component type you never spawn is copied in full forever — which makes `maxCount` and pruning latency levers as much as memory levers. The guide described them only as memory levers; a user pruned two unused types out of a 6.31 MiB heap and cut it to 2.45 MiB — a 61% reduction that the per-tick copy follows, because the copy is one `BlockCopy` over the reserved extent — before the connection was documented anywhere. (Their absolute per-tick microseconds were Editor figures they later qualified as debug-guard inflated, so the guide quotes the ratio and tells you to measure the absolute cost in a player build.) See `Docs/ECSMemoryOptimization.md` §1.

- **The RTS profile in the configuration guide now states the navigation ceiling.** The profile recommends `MaxEntities` 1024+, while the stock navigation layer is sized for tens of agents and does not fail loudly past that: `ResolveCollisions` separates at most 64 agents per tick (the rest are silently skipped), ORCA neighbour selection is an uncapped O(N²) scan, pathfinding has no per-tick global budget, and a corridor is capped at 128 triangles. The note also explains why these are compile-time constants (they are determinism inputs) and what to build on top for unit counts in the hundreds. See `Docs/SimulationConfigGuide.md` §3.4.

## [0.10.0] - 2026-08-29

### Views — a snapshot view draws verified state, and only verified state

- **Both interpolation endpoints are now guaranteed Verified.** `RenderClock.VerifiedBaseTick` could run past `LastVerifiedTick` — from 50 ms of drift onward at a 25 ms tick with `delay 2` — and `FrameRingBuffer` hands back predicted, rollback-mutable frames without complaint, so the view drew prediction on the path that exists to avoid it. The clock now clamps its render-time scalar (after the ±10-tick catch-up snap) to hold `base + 1 <= LastVerifiedTick`, and `GetSnapshotPose` invalidates either slot past the boundary — which the clamp cannot cover at `LastVerifiedTick == 0`, nor for the one frame after a backward move of `LastVerifiedTick` (desync ladder, FullState restore, spectator resync). An invalidated slot routes into the existing single-endpoint hold or the newest-Verified fallback. `InterpolationDelayTicks` now documents `lastVerified - delay` as the target the clock converges on rather than a value it holds, and `Timescale` as the *requested* rate. Godot mirrors it.

- **A snapshot view keeps drawing after the local prediction has destroyed the entity.** The live-Predicted existence check was a gate for both paths — correct for CSP, wrong for the snapshot path, where an entity killed by prediction before the server confirmed it froze on screen for the entire lead. It is now a fallback condition: only "no data on either timeline" writes nothing. The root transform (`Uninterpolated*`, the collision/raycast reference) takes the window's `alpha = 0` endpoint instead of the predicted pose, and `param.Teleported` comes from the interpolator's verdict.

- **The pose fallback order is now window → birth pose → newest Verified → Predicted → write nothing.** The old fallback went straight to the live Predicted pose, `lead + delay` ticks ahead of the render clock, so a freshly spawned remote entity appeared, advanced, then snapped *backward* when the window reached its birth tick (≈100 ms at `delay 2`, `lead 4`). `GetSnapshotPose` now scans between the window and the newest Verified frame and holds the oldest frame carrying the entity — the pose the window will show first. Predicted is reached only when no Verified frame exists at all (session start, late join before the ring fills, FullState restore). The scan is bounded by the verified boundary and by the frame ring. Only entities that move immediately after spawning are affected; Godot mirrors it.

- **A snapshot-interpolated view now outlives its entity until the render clock reaches the death.** Lifetime was decided from the Verified frame while the view rendered `delay` ticks behind it, so destroying on the lifetime criterion silently discarded the last `delay` ticks of motion. CSP views are untouched — their lifetime tick and render tick are the same. Four consequences:
  - **The active-view dictionary is keyed by `EntityRef.ToId()` instead of `EntityRef.Index`**, so a dying view and the new occupant of its slot coexist as ordinary entries; with the Index alone, `TrySpawn`'s rebind destroyed the dying view as soon as the Predicted pass saw the new occupant. `_pendingEntityVersion` is gone with it, and with it the branch where a second in-flight spawn cancelled the first's completion — reachable only when `EntityViewFactory.CreateAsync` is overridden with a genuinely asynchronous load. No public member exposed the dictionary.
  - **A view inside the grace no longer receives `OnUpdateView`** — the entity it draws can no longer be looked up in the frame. Per-frame interpolation continues; per-tick logic hung off that hook stops `delay` ticks before the view disappears.
  - **A view inside the grace stays in `PlayerViewRegistry`, and `OnLocalViewUnregistered` fires when it is destroyed.** Unbinding on the way in left the player unmapped for the rest of the match when a rollback resurrected the entity, since re-registration happens only at the end of a spawn. `Unregister` still removes the entry only if it is the same instance.
  - **Godot's `DestroyStale` compares `EntityRef.Version`, as Unity always has** — by Index alone a reused slot never produced a stale verdict, and the two adapters would destroy views at different times.

### Views — the right entity, the right pose

- **Snapshot interpolation, the live-frame check and the CSP `prev` endpoint all compare `EntityRef.Version` now.** `Frame.Has` and `GetReadOnly` forward `entity.Index` alone, so a recycled slot answers about the **new** occupant and reports success. Three silent consequences: a new view rendered the *old* occupant for the whole interpolation window, a stale view followed the *new* occupant, and the CSP lerp's `prev` endpoint slid a new view in from a stranger's position when a destroy and a create landed in the same tick (the free list is LIFO). One shared `Occupies(frame, entity)` replaces the four checks; a mismatch invalidates that frame and the existing fallbacks handle it. Godot mirrors it. Short-lived, high-churn entities reach slot reuse first.

- **A teleport is no longer smeared across a tick, on either render path.** (1) The snapshot interpolator now holds the near endpoint when the two window endpoints' `TransformComponent.TeleportTick` differ, putting the jump on the tick boundary. (2) The CSP path compares `TeleportTick` against the tick it renders, instead of relying on `param.Teleported`, which requires `EnableErrorCorrection` *and* a rollback in the same frame — an ordinary respawn is neither, so every configuration lerped through it; only a stamp greater than zero counts, since the field defaults to `0`. (3) The snap branch is CSP-only now: it was snapping snapshot views to the predicted pose, a render delay ahead. Godot had no snap branch and gained one.

- **`_interpolationTarget` offsets are written in world space, and cleared when a pooled view comes back.** The child offset is a world-space delta that was assigned to `localPosition`, so the world position came out off by the root's yaw. Separately, activating a pooled view reset the error-visual accumulator but not the child transform, so a reused view wore its previous life's offset — one frame normally, permanently under `DisablePositionUpdate`. Both paths now share one reset, which restores what the prefab authored on the child rather than zero. Nothing here sets an interpolation target, so this was latent. The field's docs state the cost: the target trails the root by the render delay, so picking and raycasts should target the mesh.

- **A view is spawned at its entity's pose, not the prefab's — or the previous tenant's.** `IEntityViewPool.Rent` takes a position and rotation and the default `CreateAsync` was not passing them. The same frame's first `ApplyTransform` usually hides it, but that line does not run under `DisableUpdate` and skips the position under `DisablePositionUpdate`, and the view then stays wrong for good. Neither flag is set anywhere here. Godot has it too, without an interface change: its `Rent` is synchronous and returns a node not yet in the tree.

- **A spectator owns no entity.** A spectator has no `LocalPlayerId`, the engine falls back to `0`, and `0` is a real player id — the one a P2P host holds — so spectating a P2P session put the host's entity on the CSP path while everything else was on the snapshot path: it ran ahead of the world and snapped on every rollback. The same comparison existed twice, in `GetViewFlags` (render) and `IsPredictedRender` (lifetime and binding), so fixing one alone would give a view bound as verified and flagged as predicted; they are one method now, with the spectator guard inside. An SD client whose `LocalPlayerId` is `0` still renders its own entity predicted.

### Error correction — wired where it was already advertised

- **`EnableErrorCorrection` does something in P2P now.** `CapturePreRollbackTransforms` decided "a rollback is about to run" by peeking at the pending verified queue, which only the SD client fills — so everywhere else the three error-correction functions did nothing, P2P worst of all, since every view there renders on the CSP path, the only path that applies the deltas. The rollback signal is now a call-site argument, because each mode expresses it differently. Capture runs after the tick loop and before the flush so this frame's motion does not leak into the delta; the clear runs before the resync early-return so a stale delta cannot re-accumulate up to the teleport threshold. The spectator's capture/compute are removed rather than revived — every spectator view takes the snapshot path, which skips deltas by design.

- **A FullState resync now feeds the view's error correction instead of jumping in one frame.** The capture/compute pair sits in `Update`, and a resync arrives on a transport callback that was never wrapped. The assumption that a resync is past the teleport threshold anyway is false — three induced SD resyncs jumped 4.6, 5.7 and 12.1 cm, nowhere near the 1 m threshold, because the FullState restores the authoritative pose and the jump is just the prediction error at that instant. Both handlers are wired (P2P and the SD client; bootstrap and late join have no local pose to diff against), and the deltas are parked and published right after the frame's `ClearErrorDeltas`. Three lifetime rules make the two sources compose: the frame's clear runs above every early return; a frame producing both a resync correction and a rollback delta adds them, where the second used to overwrite the first; and the pre-rollback baseline is consumed by the computation that reads it. The whole correction state is cleared by `Stop()` and by an SD late join.

- **The correction offset is applied on the frame that computed it, not the frame after.** The view read `param.ErrorVisual*` *before* ticking the smoother, so the frame a delta arrived on rendered the jump at full size and the next frame jumped back — one discontinuity turned into two. Reading now happens after the tick, which shrinks the seam rather than closing it: for a 0.3 m correction, ≈0.055 m left at 60fps instead of 0.30 m followed by 0.24 m back. Godot had the same ordering and the same fix, and there it also removes a leftover offset on teleport frames. The documented decay-to-zero time is corrected to ≈1.5 s.

- **A game that leaves `EnableErrorCorrection` off no longer pays for the view-side smoother.** Unity ran two dictionary lookups and a five-stage smoother per view per frame for a feature that is off by default; Godot had already gated it. The flag is read once per frame and shared by the three dependent spots, which straddle the transform write. The `TeleportTick` half of the teleport verdict stays outside the gate — that stamp is the game's. Dead accumulation stops on both adapters, since the snapshot path discards both offsets.

- **`SimulationConfigGuide` no longer prescribes a switch that does nothing on its own.** Three profile tables recommended `EnableErrorCorrection: true` as the answer to jitter, but the flag only arms the view-side smoother and the deltas it smooths exist only for entities the game marks with `ErrorCorrectionTargetComponent`. A new §1.1 states the three touches: the authority's config wins (an SD guest runs the server's value); the rule for marking is "can this render on the CSP path", which makes it pointless for a spectator and for an SD client's remote entities and correct for all of P2P; and the marker is transparent to the state hash but not to a FullState, so peers can disagree about who carries it while every sync check passes. The parameter table gained the row for the flag.

### Wiring a view layer — the silent failures

- **A prefab's `ViewFlags` survive the spawn — the factory's answer is merged, not assigned.** The factory can express only two of the sixteen combinations and its result was assigned wholesale, so any unrelated bit the prefab set was lost. A plain OR is not the fix either: a prefab turning on `EnableSnapshotInterpolation` would force the local player's entity onto the verified path, rendering it 5–7 ticks in the past. The composition is a mask — `(prefab & ~owned) | (factory & owned)` — with ownership declared by the new virtual `FactoryOwnedFlags`. Godot is unchanged: its `ViewFlags` carries no `[Export]`.

- **`EntityViewUpdater.Initialize` takes a factory and a pool, warns when the factory is missing, and reclaims the previous session.** Both were private `[SerializeField]`s with no code path to set them, and two places here used reflection to work around it. `null` means "leave as is", not "fall back to the serialized field", so a later argument-less call cannot undo an earlier injection. The pool lives in a separate non-serialized field, because Unity cannot serialize an interface — which is what makes the async-loading and Addressables-warmup pools `IEntityViewPool` documents actually wireable. A missing factory used to return early from the collect pass with nothing logged, leaving an empty screen as the only symptom of the most common first-wiring mistake. Re-initialising used to leave the previous session's views on screen drawing a stopped engine's last frame, with keys colliding with the new session's, so `Initialize` now begins with `Cleanup()` (idempotent) and reports what it reclaimed. Note that `EntityViewFactory.Attach` writes the engine and pool onto the ScriptableObject asset, so one updater owns one factory asset.

- **A view destroyed from outside no longer takes the updater down with it.** Every loop over the active views assumed its entries were alive, so a view destroyed externally threw out of the per-frame pass — and the teardown that would have cleaned it up was one of the loops that could not finish. Such an entry is now recognised, reported once, and reclaimed on the next reconcile. Godot mirrors it with `IsInstanceValid`.

- **A destroyed factory or pool reference is treated as missing everywhere, and a factory that refuses a spawn says so.** Unity's `==` knows an object has been destroyed; `??` and `?.` do not, and `Initialize` used both — so a Missing factory produced the "no factory assigned" warning and was then attached and used anyway, while every later check disagreed and rendered nothing. A factory returning `null` from `CreateAsync` — a legitimate "do not render this" — was silent while the updater re-dispatched the spawn every tick forever; it now warns once and names what to do instead.

- **`BindBehaviour`'s enum docs no longer claim the prefab value is a default the factory overrides.** On the updater path the factory's answer covers the whole enum and is assigned whole, unlike `ViewFlags`, where only the owned bits are.

### Configuration

- **`InterpolationDelayTicks` is validated to [1, 4] instead of [1, 3], and `SimulationConfigGuide` derives the value instead of suggesting one.** The render clock rides a sawtooth of one tick plus one render frame and the slack absorbing it is `(delay − 1)` ticks, so a clamp-free render needs `frameMs ≤ (delay − 2) × tickMs`. That makes 1 and 2 unusable at any tick rate, 3 sufficient for a 60fps frame at any tick interval of 17 ms or more, and 4 necessary at 16 ms. The old cap made a 60 Hz tick unsatisfiable — which is what both SD sample servers shipped — and the guide's own "+1" adjustments named values its own validation rejected. The genre tables, the `+1` notes and the console tick suggestion are corrected, and the formula is stated with the other key formulas.

- **A P2P host now fails to start on an invalid `SimulationConfig` instead of logging and continuing.** `KlothoEngine.Initialize` decides whether to fail fast from `IKlothoNetworkService.IsHost`, which is still `false` there for a P2P host — `CreateRoom` sets it, and `HostGame` calls that *after* the engine exists. So the one participant whose config every guest ends up running was the one that never validated it. `KlothoSessionFlow.StartHost` (and `StartHostAndListen`) now calls `Validate()` before building anything. No config here is affected.

- **Joining a session logs which `SimulationConfig` governs it.** A joining client runs the authority's config and its local asset chooses nothing but the role, so adjusting `TickIntervalMs` or `InterpolationDelayTicks` in the inspector produced neither an effect nor a warning. A field-by-field "your local value differs" warning is not possible today: on the join path the local config never reaches the engine.

### Diagnostics

- **The view's `no delta for 300 frames` warning is gone.** It asked whether the entity carried `ErrorCorrectionTargetComponent` but only armed after receiving at least one delta — and deltas exist only for entities that carry it, so the gate was already the answer. What it fired on was steady input, which the predictor gets right: one live SD session warned about an entity that had received 28 `[EC][Visual]` deltas. That question is session-wide and belongs to the engine, where `CapturePreRollbackTransforms` now detects it directly. The `[EC][Visual]` debug line stays.

- **The engine's "no correction target" warning needs eight consecutive empty rollbacks, not one.** It inspects the *pre*-rollback frame, and an SD client running `UsePrediction=false` learns about the spawn from the very rollback being captured — so the first rollback of an SD session sees a world with no characters in it, and both Brawler SD clients printed the warning at tick 12. A real misconfiguration stays empty for every rollback there will ever be; the count resets on any rollback that finds a target.

- **Dev-only guards now include `DEBUG`.** 46 blocks were gated `#if DEVELOPMENT_BUILD || UNITY_EDITOR`, which `dotnet` defines under *neither* configuration, so that code did not exist in a dotnet build at all. Adding `DEBUG` exposes 41 `Runtime/` blocks to a dotnet Debug build (Release unchanged) and makes them testable. `Tools/pack-godot-addon.sh` and `deploy-addon-to-samples.sh` take a `Debug` argument, since a Release addon compiles those diagnostics out. Inside such a block the log level gates the message but not what it is fed: an argument computed beforehand runs regardless — the SD prediction line was walking the whole command buffer that way and now asks the level first.

- **A stalled input chain no longer prints one WARN per tick.** `[ChainBreak]` throttled on the stalled tick, and the dominant case — a peer's input arriving one tick late — advances that tick every tick, so the guard never held (~80 lines for a 2 s warmup at `InputDelayTicks=0`, 25 ms tick). It now keys on **which players are missing**: one line immediately, one per second while the set is unchanged, an immediate line when it changes, and the swallowed count carried as `(+N suppressed since tick=M)`. The mask is computed without allocating.

- **The bootstrap FullState broadcast is stamped `FullStateKind.InitialState`, and a peer that already holds that state drops it quietly.** A peer present at game start built the identical snapshot itself, so dropping the host's copy is correct — but it was warned as an anomaly once per guest per session, which made `unexpectedFullStateDrop` mean nothing. Late joiners arrive with `_expectingFullState` set and are unaffected. The drop is also the one free chance to compare tick-0 state across peers, so a **hash mismatch is now an error**: it means the two sides diverged before the first tick, which no later desync report can attribute. The local tick-0 hash is taken once per session and cleared on `Stop()`.

### Breaking changes

- **`ViewFlags.UseCachedTransform` is removed, and `1 << 2` stays vacant.** The flag promised to "skip per-tick recalculation" and nothing read it — the view transform pipeline moved from per-tick to per-frame, so there is nothing left to skip. Source-breaking for anyone naming it; **wire- and asset-compatible**, because the bit is left empty. Do not renumber the remaining flags to reclaim it: `_viewFlags` is serialized as an int on the prefab, so pulling `EnableSnapshotInterpolation` down to `1 << 2` would change what an existing asset means. Removed from the Godot `ViewFlags` too.
- **`ErrorCorrectionSettings` loses seven fields it never used.** `MinRate`, `MaxRate`, `PosBlendStart`, `PosBlendEnd`, `PosTeleportDistance`, `RotTeleportDeg` and `SmoothingRate` are gone; `PosMinCorrection` and `RotMinCorrectionDeg` remain. The engine only ever read those two — the type is a delta filter, and everything the removed names describe belongs to the view's `ErrorVisualState`, where all seven existed **verbatim**, so raising one on the engine did nothing while an identically named knob did the work. Source-breaking for anyone assigning those fields; nothing else, as the struct carries no `[Serializable]` and appears in no asset, prefab or wire message. Nothing here ever assigned `IKlothoEngine.ErrorCorrectionSettings`, so the two live thresholds have always been their defaults.
- **`UpdatePositionParameter.DeltaTime` is removed.** Nothing read it, and it was not an extension point worth keeping: the value is `Time.deltaTime`, a global static, so an override reads it directly. There is no Godot counterpart to this struct — that adapter applies the transform inline.

### API additions

- **`EntityViewFactory.FactoryOwnedFlags`** (virtual, defaults to `EnableSnapshotInterpolation`) and **`ComposeViewFlags(prefabFlags, factoryFlags)`** — the ownership mask and the merge above. Override the property to claim another bit rather than bypassing the merge.
- **`EntityViewUpdater.Initialize(engine, factory = null, pool = null)`** — the five existing `Initialize(engine)` call sites compile unchanged.
- **`EntityViewFactory.IsPredictedRender(ownerId, hasOwner)`** — the single owner comparison behind both the render flags and the bind behaviour. The one-argument form stays as a `hasOwner: true` overload.
- **`EntityViewFactory.TryGetSpawnPose(frame, entity, out pos, out rot)`** (`protected static`, both adapters) — the pose the default spawn path applies, made reachable by an override that returns a view without calling base. Adopting a scene-placed object is why it exists.
- **`VerifiedFrameInterpolator.GetSnapshotPose(entity, engine)`** returning the new **`SnapshotPose`** (`Position`, `Rotation`, `BasePosition`, `BaseRotation`, `Occupied`, `Teleported`) — one query where the position and rotation helpers each did their own, halving the ring lookups per entity per frame and assembling the render clock once instead of four times. `InterpolatePosition` / `InterpolateRotation` remain as wrappers.

### Samples

- **Brawler's two config profiles now agree where they should.** The client asset moves to a 25 ms tick to match the server, and both move to `InterpolationDelayTicks: 3`. The two fields are coupled — the slack absorbing the render clock's sawtooth is `(delay − 1)` ticks while the sawtooth is one tick at any tick rate. Three live runs: 50 ms / delay 3, clamp engaged on 0.2–0.3% of frames; 25 ms / delay 2, 7.4–9.1%, held up to 231 ms; 25 ms / delay 3, one frame. `UsePrediction` and `SDInputLeadTicks` still differ between the two files on purpose.
- **The Godot P2P sample actually enables error correction.** It needed the flag on the game node *and* `ErrorCorrectionTargetComponent` on the spawned entities — the engine's new missing-target warning found the second. The zero input delay and pinned debug log level in it are a measurement stimulus, not a configuration to inherit.
- **Brawler's moving platform can be rendered by the standard pipeline.** Its view subclassed `EntityView` and then bypassed all of it — own binding fields, a hand-written lerp in a magic `LateUpdate` with the traps listed above — and never ran anyway, because the prefab was in no scene. The updater now adopts scene-placed views: the factory returns the scene instance instead of instantiating and deactivates instead of destroying, after which the view receives everything a spawned view does. Which scene object adopts which entity is fixed by name order rather than scene-scan order. `GameDevWorkflow` Step 7 documents the pattern and the five things that bite: the `Destroy` override is mandatory (the default hands the view to the pool, which destroys anything it did not lend); `TryGetBindBehaviour` and `GetViewFlags` must be overridden together (they are evaluated independently, so changing one gives a predicted lifetime with a verified render); the scene references are a runtime hand-off that must include inactive objects; view initialisation has to wait for the stage scene to load; and adoption can fail, since with fewer scene objects than entities the factory has to decide. The stage scenes still ship without a platform instance placed.
- **`SdSample` and `P2pSample` player views no longer override `OwnerMatches`.** Neither sample's entities carry `OwnerComponent` and the updater short-circuits to `true` without one, so those overrides never ran — the shape was being copied out of the one sample that legitimately needs it. One `OwnerMatches` override is left in the repository.

### Tooling

- **The code generator emits factory and warmup registrations in type-name order.** The compilation hands the emitters syntax-tree order, which differs per project for identical sources, so the two builds wrote the same registrations in different orders and a layout comparison read it as drift. Registration order carries no wire meaning, so the regenerated output is otherwise identical.
- **`run-all-tests.sh` fails when a suite did not run, and includes Release in the default run.** A suite that produced no result file vanished from the summary, so a run where Brawler EditMode failed to compile still printed "All passed" (the exit code was correct; the summary was not). The summary now derives which suites the run intended to produce and reports the missing ones with a non-zero exit; an empty run is a failure. The default was dotnet Debug alone, which compiles out everything behind `DEBUG` / `DEVELOPMENT_BUILD` / `UNITY_EDITOR` — 23 tests in the runtime suite differ between configurations and the Release side had never run. `--debug` restores the old behaviour at half the time, and `--no-build` now means "both configurations are already built", checking that up front and naming the one that is not.

### Docs

- **`Docs/ViewInterpolation.md` — a standalone view-interpolation guide, written for a first reader.** Everything this release touched was documented already, but spread across `GameDevAPI` §7, `GameDevWorkflow`, `FEATURES` and `SimulationConfigGuide`, in the order the engine needs rather than the order a game developer meets it. The new page takes the other order: why interpolation exists at all, the two render paths and who gets which, the render clock and how to choose `InterpolationDelayTicks`, the snapshot fallback ladder and the despawn grace, the one line of simulation code a teleport owes the view, error correction's three touches, and a symptom → cause → fix table. No new material — the details stay in the documents it points at. Indexed from the README table and from `Cookbook` §1.
