...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    /// </summary>
    [Collection(EcsRegistryCollection.Name)]   // folds over the process-global component registry — see EcsRegistryCollection
    public sealed class DesyncDiagnosticsPhase1Tests
    {
        ...
        [Fact] // Total/FrameHash must equal the manual fold of the exposed layers — the probe/tool contract
        public void Breakdown_TotalEqualsFoldOfExposedLayers()
        {
            ...
            Assert.Equal(b.FrameHash, frameFold);
            ...
            Assert.Equal(b.Total, (long)totalFold);
            Assert.Equal(total, b.Total);
            Assert.True(b.SystemHashes.Length > 0, "harness must register a snapshot participant");
        }

        // ── localization ─────────────────────────────────────────────

        [Fact] // mutating one component moves exactly that type's layer hash — nothing else
        public void ComponentMutation_LocalizesToThatTypeOnly()
        {
            ...
            Assert.Equal(b1.Total, b2.Total);   // identical setups agree end to end
            ...
            Assert.NotEqual(b1.Total, b2.Total);
            Assert.NotEqual(b1.ComponentHashes[transformIdx], b2.ComponentHashes[transformIdx]);
            for (int i = 0; i < b1.ComponentHashes.Length; i++)
            {
                if (i == transformIdx) continue;
                Assert.Equal(b1.ComponentHashes[i], b2.ComponentHashes[i]);
                Assert.Equal(b1.ComponentCounts[i], b2.ComponentCounts[i]);
            }
            Assert.Equal(b1.SystemHashes, b2.SystemHashes);   // system layer untouched
        }

        [Fact] // mutating only participant state moves ONLY the system layer — the blind spot pin:
        ...
        public void SystemStateMutation_LocalizesToSystemLayerOnly()
        {
            ...
            Assert.NotEqual(b1.Total, b2.Total);
            Assert.Equal(b1.ComponentHashes, b2.ComponentHashes);   // component layer fully clean
            Assert.NotEqual(b1.SystemHashes[0], b2.SystemHashes[0]);
        }

        // ── resync-critical symmetry ─────────────────────────────────

        [Fact] // GetStateHash() must equal the FullState serialization hash — the resync apply-verification
        ...
        public void GetStateHash_EqualsFullStateSerializationHash()
        {
            ...
            Assert.Equal(simP.GetStateHash(), fullHashP);
            ...
            Assert.Equal(simF.GetStateHash(), fullHashF);
        }

        // ── ring semantics ───────────────────────────────────────────

        [Fact] // re-record for the same tick overwrites the slot — the mechanism rollback resim relies on
        ...
        public void HashHistory_ReRecord_OverwritesWithCurrentTimeline()
        {
            ...
            Assert.True(sim.TryGetHashHistory(5, out long speculative, out _));
            ...
            Assert.True(sim.TryGetHashHistory(5, out long corrected, out _));
            Assert.NotEqual(speculative, corrected);
            Assert.Equal(sim.GetStateHash(), corrected);
        }

        [Fact] // wraparound invalidates the overwritten tick instead of serving another tick's values
        public void HashHistory_Wraparound_InvalidatesStaleSlot()
        {
            ...
            Assert.True(sim.TryGetHashHistory(0, out _, out _));

            sim.ComputeAndRecordHashHistory(4);   // slot 0 reused by tick 4

            Assert.False(sim.TryGetHashHistory(0, out _, out _));
            Assert.True(sim.TryGetHashHistory(4, out _, out _));
        }

        // ── zero-GC gate ─────────────────────────────────────────────

        [Fact] // the steady accumulation path (hash + breakdown fill + ring copy) must allocate nothing —
        ...
        [Fact] // command serialization must round-trip byte-stably: the SD classification digests the
        ...
        public void CommandSerialization_RoundTrip_ByteStable()
        {
            ...
            Assert.Equal(first, second);   // byte-identical ⇒ FNV digests identical on both ends
        }
