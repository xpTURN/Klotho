...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    public class NavMeshObstacleExtractorTests
    {
        ...
        public void G1_RoomWithPillar_OuterCW_HoleCCW_ConvexityCorrect()
        {
            ...
            Assert.Equal(2, offs.Length);          // two rings
            Assert.Equal(8, v.Length);             // 4 + 4

            int r0s = offs[0], r0e = offs[1];
            int r1s = offs[1], r1e = v.Length;
            Assert.Equal(4, r0e - r0s);
            Assert.Equal(4, r1e - r1s);

            // outer boundary (walkable inside) is CW; pillar hole (walkable outside) is CCW
            Assert.True(SignedArea2(v, r0s, r0e) < 0, "outer ring should be CW");
            Assert.True(SignedArea2(v, r1s, r1e) > 0, "pillar ring should be CCW");
            ...
            for (int i = r0s; i < r0e; i++) Assert.False(o[i].isConvex, $"outer[{i}]");
            // pillar corners are convex obstacle spikes → isConvex true
            for (int i = r1s; i < r1e; i++) Assert.True(o[i].isConvex, $"pillar[{i}]");
        }
        ...
        public void G4_ExtractedSegments_MatchBoundaryEdges_AndWindingCorrect()
        {
            ...
            Assert.Equal(CountBoundaryEdges(nav), v.Length);

            // (b) orientation: outer CW + hole CCW encodes "walkable on the right" per edge.
            Assert.True(SignedArea2(v, offs[0], offs[1]) < 0);
            Assert.True(SignedArea2(v, offs[1], v.Length) > 0);
        }
        ...
        public void G6_Deterministic_IndependentExtractions_Identical()
        {
            ...
            for (int i = 0; i < v1.Length; i++)
                Assert.True(v1[i] == v2[i], $"vertex {i} differs"); // exact FP64
        }
        ...
        public void G7_PinchVertex_SplitsIntoTwoRings()
        {
            ...
            Assert.Equal(2, offs.Length);   // two independent rings, not one figure-8
            Assert.Equal(8, v.Length);      // 4 + 4
            Assert.Equal(4, offs[1] - offs[0]);
            Assert.Equal(4, v.Length - offs[1]);
        }
        ...
        public void G2_LShape_ElbowIsObstacleConvex_RestReflex()
        {
            ...
            Assert.Equal(1, convexCount); // exactly the elbow is obstacle-convex
            // elbow is the (4,4) inner corner
            Assert.True(System.Math.Abs(o[elbow].point.x.ToFloat() - 4) < 1e-3f
        ...
        public void G3_AgentInsideRoom_StaysInside_MultiTick()
        {
            ...
            Assert.True(maxX < 9.7, $"agent left the room through +X wall, maxX={maxX}");
            ...
            Assert.True(maxX > 8.0, $"agent never approached the +X wall it was driving at (maxX={maxX} — jam?)");
        }
        ...
        public void G9_DualSource_ConcatMerge_BothObstaclesActive()
        {
            ...
            Assert.True(av.DebugObstacleCount == nv.Length + block.Length);
            Assert.True(av.DebugNumObstLines > 0, "block/wall should produce obstacle lines");
        }
        ...
        public void S2_LoadNavMeshObstacles_WiresObstaclesIntoAvoidance()
        {
            ...
            Assert.Equal(0, agentSystem.DebugObstacleCount);
            ...
            Assert.True(agentSystem.DebugObstacleCount > 0, "helper should load NavMesh obstacles");
            Assert.Equal(av.DebugObstacleCount, agentSystem.DebugObstacleCount);
            ...
            var frame = sim.Frame;
            av.ComputeNewVelocity(0, ref frame, e, 1, DT);
            Assert.True(av.DebugNumObstLines > 0, "wall near agent should produce obstacle lines");
        }
        ...
        public void S2b_HelperDeterministic_IndependentWirings_Identical()
        {
            ...
            Assert.Equal(av1.DebugObstacleCount, av2.DebugObstacleCount);
            var o1 = av1.DebugObstacles;
            var o2 = av2.DebugObstacles;
            for (int i = 0; i < av1.DebugObstacleCount; i++)
            {
                Assert.True(o1[i].point == o2[i].point, $"obstacle {i} point differs");
                Assert.Equal(o1[i].isConvex, o2[i].isConvex);
                Assert.Equal(o1[i].nextIndex, o2[i].nextIndex);
            }
        }
