...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    public class NavObstacleClearanceTests
    {
        ...
        public void Reachability_LCorridor_ObstacleOnReachesLikeOff()
        {
            ...
            Assert.True(off.minD < 0.6, $"baseline (avoidance OFF) should reach the corner destination (min {off.minD:F2})");
            Assert.True(on.minD < 0.6, $"avoidance ON (runtime default) must not stall on the corridor corner (min {on.minD:F2})");
        }
        ...
        public void CornerSlowdown_RemovedByObstacleRadiusInset()
        {
            ...
            Assert.True(uncorrected.minCruiseSpeed < 2.5,
            ...
            Assert.True(corrected.minCruiseSpeed < double.MaxValue, "corrected run never entered the cruise window");
            Assert.True(corrected.minCruiseSpeed > 3.0,
                $"inset-corrected run must not dip at the corner (min speed {corrected.minCruiseSpeed:F2} " +
                $"vs OFF {off.minCruiseSpeed:F2})");
            Assert.True(corrected.minD < 0.6, "inset-corrected run must still reach");
        }
        ...
        public void BakeAgentRadius_RoundTripsAndAutoAppliesAsInset()
        {
            ...
            Assert.Equal(FP64.Half.RawValue, built.BakeAgentRadius.RawValue);
            ...
            Assert.Equal(FP64.Half.RawValue, loaded.BakeAgentRadius.RawValue);
            Assert.Equal(FP64.FromInt(45).RawValue, loaded.BakeMaxSlopeDeg.RawValue);
            Assert.Equal(FP64.FromInt(2).RawValue, loaded.BakeAgentHeight.RawValue);
            Assert.Equal(FP64.FromDouble(0.75).RawValue, loaded.BakeAgentClimb.RawValue);
            ...
            sys.SetAvoidance(av);
            sys.LoadNavMeshObstacles();
            Assert.Equal(FP64.Half.RawValue, av.ObstacleRadiusInset.RawValue);
        }
        ...
        public void EffectiveRadiusZero_OnBoundary_NoCrash()
        {
            ...
            foreach (var p in positions)
            {
                ...
                Assert.True(FP64.Abs(result.x) < FP64.FromInt(100) && FP64.Abs(result.y) < FP64.FromInt(100),
