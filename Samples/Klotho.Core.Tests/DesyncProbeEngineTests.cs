...
{
    /// <summary>
    /// Serializes the test classes that drive the component registry. <c>ComponentStorageRegistry</c> is
    /// process-global and grows when an engine initializes (WarmupRegistry), so a class doing that in
    /// parallel with a class folding over <c>RegisteredTypeIdsSorted</c> can grow the registry mid-fold.
    /// Production never sees this — the registry is frozen for the process — but xUnit runs classes
    /// concurrently by default, so the ones that touch it have to share a collection.
    /// </summary>
    [CollectionDefinition(EcsRegistryCollection.Name)]
    public sealed class EcsRegistryCollection
    {
        public const string Name = "EcsRegistry";
    }
    ...
    /// </summary>
    [Collection(EcsRegistryCollection.Name)]
    public sealed class DesyncProbeEngineTests
    {
        private const int MaxEntities = 64;
        private const int RingTicks = 16;

        [Fact] // the round trip end to end: narrow to the tick, then name the component
        public void RoundTrip_LocalizesTheDivergedTickAndComponent()
        {
            ...
            var l1 = Assert.Single(net.SentRequests);
            Assert.Equal((byte)DesyncProbeLevel.TickHashes, l1.Level);
            Assert.Equal(6, l1.ToTick);
            ...
            Assert.Equal(2, net.SentRequests.Count);
            var l2 = net.SentRequests[1];
            Assert.Equal((byte)DesyncProbeLevel.Breakdown, l2.Level);
            Assert.Equal(4, l2.FromTick);
            Assert.Equal(4, l2.ToTick);
            ...
            Assert.Equal(4, verdict.Value.DivergedTick);
            Assert.Equal(DesyncLayer.Component, verdict.Value.Layer);
            Assert.Equal(ComponentStorageRegistry.GetTypeId(typeof(TransformComponent)), verdict.Value.TypeIdOrParticipantIdx);
        }

        [Fact] // M2: the verdict must pair LocalHash and RemoteHash at the SAME tick (the diverged one).
        ...
        public void Verdict_RemoteHash_IsTheDivergedTickValue_NotDetectTick()
        {
            ...
            Assert.Equal(expectedRemoteAtDiverged, verdict.Value.RemoteHash);
            Assert.NotEqual(222, verdict.Value.RemoteHash);          // the old (mislabeled) detect-tick value
            Assert.Equal(remote.Totals[4], verdict.Value.LocalHash); // pair is same-tick: local total at 4
        }

        [Fact] // the system layer is a diagnostic blind spot — the probe must reach it too
        public void RoundTrip_LocalizesADivergingSystem()
        {
            ...
            Assert.True(verdict.HasValue);
            Assert.Equal(DesyncLayer.System, verdict.Value.Layer);
            Assert.Equal(0, verdict.Value.TypeIdOrParticipantIdx);
        }

        [Fact] // L2: the requester-side probe de-dup guard (_lastProbedTickByPeer) is keyed by peerId and
        ...
        public void PlayerLeft_ClearsProbeDedupGuard_SoRecycledPeerCanProbeAgain()
        {
            ...
            net.RaiseDesyncDetected(playerId: 1, tick: 4, localHash: 1, remoteHash: 2);
            Assert.Equal(afterFirst, net.SentRequests.Count);
            ...
            net.RaiseDesyncDetected(playerId: 1, tick: 4, localHash: 1, remoteHash: 2);
            Assert.Equal(afterFirst + 1, net.SentRequests.Count);
        }
        ...
        [Fact] // same command digest both sides, state still diverged ⇒ State (determinism violation)
        public void P2P_SameCommandDigest_ClassifiesStateDivergence()
        {
            ...
            Assert.NotEqual(0L, localCmd);   // guard: factory present + tick in input retention
            ...
            Assert.True(verdict.HasValue);
            Assert.Equal(DesyncClass.State, verdict.Value.Class);
            Assert.Equal(DesyncLayer.Component, verdict.Value.Layer);
        }

        [Fact] // command digests differ ⇒ Input divergence (the peers executed different input sets)
        public void P2P_DifferingCommandDigest_ClassifiesInputDivergence()
        {
            ...
            Assert.True(verdict.HasValue);
            Assert.Equal(DesyncClass.Input, verdict.Value.Class);
        }

        [Fact] // responder could not produce a digest (tick trimmed / history off) ⇒ class withheld as
        ...
        public void P2P_ResponderDigestUnavailable_ClassUnknownButLayerReported()
        {
            ...
            Assert.NotEqual(0L, LocalCmdDigestAt(net, 4));
            ...
            Assert.True(verdict.HasValue);
            Assert.Equal(DesyncClass.Unknown, verdict.Value.Class);
            Assert.Equal(DesyncLayer.Component, verdict.Value.Layer);   // layer survives a missing class
        }

        [Fact] // no agreed tick inside the shared window ⇒ say so, do NOT name the oldest visible one
        public void NoAnchorInWindow_DegradesInsteadOfGuessingATick()
        {
            ...
            Assert.Single(net.SentRequests);                       // no L2 — there is no tick to ask about
            Assert.True(verdict.HasValue);
            Assert.Equal(DesyncLayer.None, verdict.Value.Layer);   // honest "not localized"
        }

        [Fact] // history off on the far side ⇒ conclude immediately, do not sit out the timeout
        public void UnavailableRemoteHistory_ConcludesWithoutLayer()
        {
            ...
            Assert.Single(net.SentRequests);
            Assert.True(verdict.HasValue);
            Assert.Equal(DesyncLayer.None, verdict.Value.Layer);
        }

        [Fact] // a round trip whose answer never arrives must expire on the timeout and conclude without a
        ...
        public void ProbeTimeout_ExpiresAndConcludesWithoutLayer()
        {
            ...
            Assert.Single(net.SentRequests);        // L1 in flight, awaiting a response that never comes
            ...
            Assert.Single(net.SentRequests);        // still pending — no L2, no premature verdict
            ...
            Assert.True(verdict.HasValue);
            Assert.Equal(DesyncLayer.None, verdict.Value.Layer);
            Assert.Equal(6, verdict.Value.DivergedTick);   // ConcludeWithoutLayer falls back to DetectTick (P2P offset 0)
        }

        [Fact] // registries of different sizes ⇒ every index means something else. Refuse to name a component.
        public void BuildSkew_ReportsMismatchInsteadOfALayer()
        {
            ...
            Assert.True(verdict.HasValue);
            Assert.Equal(DesyncLayer.None, verdict.Value.Layer);
            Assert.Equal(-1, verdict.Value.TypeIdOrParticipantIdx);
        }

        [Fact] // A responder that answers about a tick we did not capture means the two sides disagree on
        ...
        public void ResponseForATickOutsideTheCapture_IsRefusedLoudly()
        {
            ...
            Assert.Contains(log.Lines, l => l.Contains("probe tick space mismatch"));
            Assert.True(verdict.HasValue);
            Assert.Equal(DesyncLayer.None, verdict.Value.Layer);   // no component named off a mislabelled tick
        }

        [Fact] // correlation ids are small monotonic ints — a third peer must not be able to answer for the responder
        public void ResponseFromTheWrongPeer_IsRejected()
        {
            ...
            Assert.Single(net.SentRequests);   // no L2 was launched off the forged answer
            Assert.False(diagnosed);
        ...
        [Fact] // late, duplicated or invented correlation ids are dropped without a trace
        ...
        [Fact] // a desync fires repeatedly as it escalates — one capture per peer, not one per re-fire
        ...
        [Fact] // without local history there is nothing to diff a remote answer against — do not ask
        public void HistoryDisabled_SkipsTheProbeEntirely()
        {
            ...
            Assert.Empty(net.SentRequests);
        }

        [Fact] // serving is a ring read, never a rewind: a tick the ring lost is answered "unavailable"
        public void Responder_ServesFromTheRing_AndAnswersEmptyWhenItCannot()
        {
            ...
            var (peer, served) = Assert.Single(net.SentResponses);
            Assert.Equal(3, peer);                       // answered to the peer it came FROM
            Assert.Equal(77, served.CorrelationId);
            Assert.True(DesyncProbePayload.TryUnpackL1(served.Payload, out int[] ticks, out long[] totals));
            Assert.Equal(new[] { 10, 11, 12, 13, 14 }, ticks);
            for (int i = 0; i < ticks.Length; i++)
            {
                Assert.True(sim.TryGetHashHistory(ticks[i], out long expected, out _));
                Assert.Equal(expected, totals[i]);
            }
            ...
            var (_, empty) = Assert.Single(net.SentResponses);
            Assert.True(DesyncProbePayload.TryUnpackL2(empty.Payload, out ulong[] components, out _, out _));
            Assert.Empty(components);
        }

        [Fact] // a wire-supplied window must never spin the responder. The old serve walked `tick <= to`
        ...
        public void Responder_BoundsHostileTickWindow_AndDoesNotSpin()
        {
            ...
            var (_, a) = Assert.Single(net.SentResponses);
            Assert.True(DesyncProbePayload.TryUnpackL1(a.Payload, out int[] ticksA, out _));
            Assert.Empty(ticksA);
            ...
            var (_, b) = Assert.Single(net.SentResponses);
            Assert.True(DesyncProbePayload.TryUnpackL1(b.Payload, out int[] ticksB, out _));
            Assert.Empty(ticksB);
        }

        [Fact] // A peer that has just desynced must STILL be able to answer a probe. In P2P both peers
        ...
        public void Responder_CanStillServe_AfterItsOwnDesyncFlushedTheHistory()
        {
            ...
            net.RaiseDesyncDetected(playerId: 1, tick: 6, localHash: 1, remoteHash: 2);
            Assert.Contains(log.Lines, l => l.Contains("[Desync][Diag][History]"));   // the dump really ran
            ...
            var (_, served) = Assert.Single(net.SentResponses);
            Assert.True(DesyncProbePayload.TryUnpackL2(served.Payload, out ulong[] components, out _, out _));
            Assert.NotEmpty(components);
        }

        [Fact] // the dump is per diverged tick: a re-firing desync must not re-print the same history
        public void HistoryDump_EmitsOncePerDivergedTick()
        {
            ...
            Assert.Single(log.Lines.FindAll(l => l.Contains("[Desync][Diag][History]")));
        }

        [Fact] // a response must never route by the wire's RequesterPlayerId — only by the transport peer
        public void Responder_IgnoresTheClaimedRequesterPlayerId()
        {
            ...
            var (peer, _) = Assert.Single(net.SentResponses);
            Assert.Equal(2, peer);   // the transport peer, not the claim
        }

        // ── verdict validation (SD server side) ───────────────────────

        [Fact] // the index means a different thing per layer, so it is bounded per layer — one shared
        ...
        public void Verdict_IndexIsValidatedAgainstTheLayerItBelongsTo()
        {
            ...
            Assert.DoesNotContain(log.Lines, l => l.Contains("[Desync][Verdict][src=client-reported"));
            ...
            Assert.Contains(log.Lines, l => l.Contains("[Desync][Verdict][src=client-reported unverified]"));
        }

        [Fact] // Unknown is a client-local degradation; an SD client always holds both digests, so it
        ...
        [Fact] // a tick far past the server's own is a bogus claim
