...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    public class NavGraphLocalObstacleTests
    {
        ...
        public void H1_ExtractorCsr_IsPartitionOverBoundarySegments()
        {
            ...
            Assert.Equal(triCount + 1, triSegStart.Length);
            Assert.Equal(verts.Length, triSegList.Length);
            Assert.Equal(verts.Length, triSegStart[triCount]); // coverage: every segment owned
            ...
            {
                Assert.True(triSegStart[t] <= triSegStart[t + 1]); // monotone
                for (int k = triSegStart[t]; k < triSegStart[t + 1]; k++)
                {
                    int seg = triSegList[k];
                    seen[seg]++;
                    Assert.True(HasBoundaryEdgeStartingAt(nav, t, verts[seg]),
                        $"triangle {t} should own a boundary edge starting at segment {seg}");
                }
            }
            for (int s = 0; s < seen.Length; s++)
                Assert.Equal(1, seen[s]); // exactly once
        }
        ...
        public void H2a_OpenRoom_GraphSelectsSameSegmentSetAsBruteForce()
        {
            ...
            Assert.True(av.DebugLastObstaclePathWasGraph, "graph path must have run");

            var graph = Selected(av);
            var brute = BruteForceSelected(nav, V(5, 0, 4), speed: 5);
            Assert.True(graph.SetEquals(brute),
        ...
        public void H2b_SplitRooms_GraphExcludesFarRoomWallThatBruteForceIncludes()
        {
            ...
            Assert.DoesNotContain(graph, InRoomB);          // graph never crosses the gap
            Assert.Contains(brute, InRoomB);                // brute-force picks the facing wall (phantom)
            Assert.NotEmpty(graph);                         // room A's own near wall still selected
        }
        ...
        public void H3_StackedFloors_GraphDropsUpperFloorPhantom()
        {
            ...
            Assert.True(AnySelectedAtFloor(brute, seg2tri, nav, 4.5, 5.5),
                "brute force should include the 2F phantom wall");
            Assert.False(AnySelectedAtFloor(graph, seg2tri, nav, 4.5, 5.5),
        ...
        public void H4_Ramp_StepDeltaTraversesBothConnectedFloors()
        {
            ...
            Assert.True(AnySelectedAtFloor(graph, seg2tri, nav, -0.5, 0.5),
                "ramp query should reach connected 1F walls in range");
            Assert.True(AnySelectedAtFloor(graph, seg2tri, nav, 3.5, 4.5),
        ...
        public void H5a_UnseededAgent_FallsBackToBruteForce()
        {
            ...
            Assert.False(av.DebugLastObstaclePathWasGraph, "unseeded (-1) agent must use brute-force");
            // Brute force at this open 1F spot sees the 2F phantom.
            Assert.True(AnySelectedAtFloor(Selected(av), seg2tri, nav, 4.5, 5.5),
        ...
        public void H5b_NonNavmeshSource_UsesBruteForce()
        {
            ...
            Assert.False(av.DebugLastObstaclePathWasGraph);
            Assert.True(av.DebugNumObstLines > 0, "brute-force should still produce obstacle lines");
        }
        ...
        public void H5c_OpenAgentUnderPhantom_GraphYieldsZeroLines_NotFallback()
        {
            ...
            Assert.True(av.DebugLastObstaclePathWasGraph, "graph path must have run (not fallback)");
            Assert.Equal(0, av.DebugNumObstLines); // graph: no reachable wall → 0 lines, no phantom
        }
        ...
        public void H6_Determinism_And_UpdateIsGcZero()
        {
            ...
            Assert.True(Selected(avA).SetEquals(Selected(avB)), "repeated identical input must select the identical set");
            ...
            Assert.Equal(avA.DebugSelectedObstacleCount, avB.DebugSelectedObstacleCount);
            for (int k = 0; k < avA.DebugSelectedObstacleCount; k++)
                Assert.Equal(avA.DebugSelectedObstacleSegments[k], avB.DebugSelectedObstacleSegments[k]);
            ...
            Assert.Equal(0, allocated / 20000); // 0 bytes per tick
        }
        ...
        public void H10_LargeMesh_TripsBfsFrontierOverflowDiagnostic()
        {
            ...
            Assert.True(nav.Triangles.Length > 256, "fixture must exceed the frontier cap");
            ...
            Assert.True(sys.DebugBfsFrontierOverflowCount > 0,
        ...
        public void H9_BakeSlope_AutoDerivesClimbCap()
        {
            ...
            Assert.False(AnySelectedAtFloor(capped, seg2tri45, nav45, 3.5, 4.5),
                "recorded 45° slope must auto-cap the climb and exclude the 2F wall");
            Assert.True(AnySelectedAtFloor(capped, seg2tri45, nav45, 1.5, 2.5),
            ...
            Assert.True(AnySelectedAtFloor(loose, seg2tri60, nav60, 3.5, 4.5),
        ...
        public void H8_ClimbCap_ExcludesUpperFloorButKeepsRampLocalWalls()
        {
            ...
            Assert.True(AnySelectedAtFloor(uncapped, seg2tri, nav, 3.5, 4.5),
            ...
            Assert.False(AnySelectedAtFloor(capped, seg2tri, nav, 3.5, 4.5),
                "climb cap must exclude the 2F wall");
            Assert.True(AnySelectedAtFloor(capped, seg2tri, nav, 1.5, 2.5),
