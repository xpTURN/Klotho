...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    {
        [Test] // stop while leaving an unverified tail → TotalTicks is LastVerifiedTick, not CurrentTick+delay
        public void TailClamp_TotalTicksEqualsLastVerifiedTick()
        {
            ...
            Assert.IsTrue(h.Engine.LastVerifiedTick < h.Engine.CurrentTick,
                $"expected an unverified tail (LastVerifiedTick={h.Engine.LastVerifiedTick}, CurrentTick={h.Engine.CurrentTick})");
            Assert.AreEqual(h.Engine.LastVerifiedTick, replay.Metadata.TotalTicks);
            ...
            for (int t = 0; t <= replay.Metadata.TotalTicks; t++)
                Assert.IsTrue(replay.GetCommandsForTick(t).Count >= 1, $"tick {t} has no recorded commands");
        }

        [Test] // after the clamp, a clean session matches the record on every tick of the replay domain (full domain, not just the intersection)
        public void CleanSession_FullReplayDomain_HashesMatch()
        {
            ...
            Assert.IsTrue(total > 8, $"replay too short (TotalTicks={total})");
            for (int t = 0; t <= total; t++)
            {
                Assert.IsTrue(replaySim.TickHashCapture.ContainsKey(t), $"replay run missing tick {t}");
                Assert.IsTrue(h.Sim.TickHashCapture.ContainsKey(t), $"record run missing tick {t}");
                Assert.AreEqual(h.Sim.TickHashCapture[t], replaySim.TickHashCapture[t]);
            }
        }

        [Test] // same-length re-record is in-place (no growth); only a different length appends
        public void RecordCommands_SameLengthReRecord_DoesNotGrowBuffer()
        {
            ...
            Assert.AreEqual(sizeAfterFirst, sizeAfterReRecords);
            ...
            Assert.IsTrue(sizeAfterGrow > sizeAfterReRecords,
