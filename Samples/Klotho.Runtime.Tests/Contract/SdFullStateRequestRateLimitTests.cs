...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    {
        [Test] // a flood collapses to a single serve — this is the amplification vector being closed
        public void RepeatedRequests_WithinCooldown_ServedOnce()
        {
            ...
            Assert.AreEqual(1, served);
        }

        [Test] // the cooldown is per-peer: one peer flooding must not starve another peer's resync
        public void Cooldown_IsPerPeer_DoesNotBlockOtherPeers()
        {
            ...
            Assert.AreEqual(new[] { 7, 8 }, servedPeers);
        }

        [Test] // disconnect clears the cooldown, so a recycled peerId is not served a stale window.
