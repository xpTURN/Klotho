...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    {
        [Test] // #1: the three paths a desync triggers must not share a window
        public void FullStateProbeAndVerdict_HaveIndependentBudgets()
        {
            ...
            Assert.AreEqual(1, fullStates);
            Assert.AreEqual(1, probes);
            Assert.AreEqual(1, verdicts);
        }

        [Test] // #2: L2 arrives ~1 RTT after L1, deep inside the window — it must still be served
        public void ProbeServe_AllowsTheSecondServeOfAnEpisode()
        {
            ...
            Assert.AreEqual(new byte[] { 1, 2 }, levels);
        }

        [Test] // an honest episode is 2 serves; a third inside the window is a flood
        public void ProbeServe_ThrottlesBeyondTheEpisodeBurst()
        {
            ...
            Assert.AreEqual(2, served);
        }

        [Test] // the window is per-peer: one peer flooding must not silence another peer's diagnosis
        public void ProbeServe_BudgetIsPerPeer()
        {
            ...
            Assert.AreEqual(new[] { 7, 7, 8 }, servedPeers);
        }

        [Test] // a fresh window reopens the budget
        public void ProbeServe_BudgetRefillsAfterTheWindow()
        {
            ...
            Assert.AreEqual(3, served);
        }

        [Test] // a verdict is a single message per episode — its own budget stays at one per window
        public void VerdictReceive_ThrottlesRepeats()
        {
            ...
            Assert.IsTrue(received <= 2, $"verdict flood must be throttled, but {received} got through");
            Assert.IsTrue(received >= 1, "the first verdict must always be received");
        }

        [Test] // peerIds are recycled: a stale window would drop the next peer's FIRST probe
        public void Disconnect_ClearsProbeAndVerdictBudgets()
        {
            ...
            Assert.AreEqual(probesBefore + 1, probes);
            Assert.AreEqual(verdictsBefore + 1, verdicts);
        }
