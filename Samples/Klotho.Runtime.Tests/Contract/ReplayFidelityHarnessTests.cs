...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    {
        [Test] // the harness actually advances ticks and records
        public void Harness_AdvancesAndRecords()
        {
            var h = new ReplayFidelityHarness();
            h.AdvanceTo(30);

            Assert.IsTrue(h.Engine.IsRecording, "clean session should be recording");
            Assert.IsTrue(h.Engine.CurrentTick >= 30, $"engine should advance past target, CurrentTick={h.Engine.CurrentTick}");
            Assert.IsTrue(h.Sim.TickCallCount > 0, "sim should have ticked");
            // Chain must actually verify (no permanent prediction stall on a clean session).
            Assert.IsTrue(h.Engine.LastVerifiedTick > h.Engine.CurrentTick - 8,
                $"verified chain lagging: LastVerifiedTick={h.Engine.LastVerifiedTick}, CurrentTick={h.Engine.CurrentTick}");
        }

        [Test] // record/replay round-trip: per-tick hashes match on every executed tick (verification apparatus holds)
        public void CleanSession_RecordReplay_PerTickHashesMatch()
        {
            ...
            Assert.IsNotNull(replayData);
            Assert.IsTrue(replayData.Metadata.TotalTicks > 0, "replay should have ticks");
            ...
            Assert.IsNotEmpty(recordCapture);
            Assert.IsNotEmpty(replayCapture);
            ...
            foreach (var kv in recordCapture)
            {
                ...
                {
                    Assert.IsTrue(kv.Value == replayHash,
            ...
            Assert.IsTrue(matched > 8, $"too few matched ticks ({matched}) — capture/replay wiring broken?");
        }
    }
}
