...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    public sealed class DesyncProbeWireTests
    {
        ...
        public void L1_RoundTrips()
        {
            ...
            Assert.IsTrue(DesyncProbePayload.TryUnpackL1(blob, out int[] outTicks, out long[] outTotals));
            Assert.AreEqual(ticks, outTicks);
            Assert.AreEqual(totals, outTotals);
        }

        [Test] // the "I have no history for that" answer — the requester must degrade, not wait out its timeout
        public void L1_EmptyAnswer_UnpacksToNothing()
        {
            byte[] blob = DesyncProbePayload.PackL1(new int[0], new long[0], 0);

            Assert.IsTrue(DesyncProbePayload.TryUnpackL1(blob, out int[] ticks, out long[] totals));
            Assert.IsEmpty(ticks);
            Assert.IsEmpty(totals);
        }
        ...
        public void L1_CountNotMatchingByteLength_IsRejected()
        {
            ...
            Assert.IsFalse(DesyncProbePayload.TryUnpackL1(truncated, out _, out _));

            byte[] padded = new byte[blob.Length + 1];
            System.Array.Copy(blob, padded, blob.Length);
            Assert.IsFalse(DesyncProbePayload.TryUnpackL1(padded, out _, out _));
        }

        [Test] // a huge count with no bytes behind it must not make the reader allocate for it
        public void L1_OversizedCount_IsRejected()
        {
            ...
            Assert.IsFalse(DesyncProbePayload.TryUnpackL1(blob, out _, out _));
        }

        [Test] // a count that overflows int32 in the length check (4 + count*12) must not slip through
        ...
        public void L2_RoundTrips()
        {
            ...
            Assert.IsTrue(DesyncProbePayload.TryUnpackL2(blob, out ulong[] outHashes, out int[] outCounts, out ulong[] outSystems));
            Assert.AreEqual(componentHashes, outHashes);
            Assert.AreEqual(componentCounts, outCounts);
            Assert.AreEqual(systemHashes, outSystems);
        }

        [Test] // an unavailable breakdown is an empty one, and it must survive the round trip as such
        public void L2_EmptyAnswer_UnpacksToNothing()
        {
            byte[] blob = DesyncProbePayload.PackL2(new ulong[0], new int[0], new ulong[0]);

            Assert.IsTrue(DesyncProbePayload.TryUnpackL2(blob, out ulong[] hashes, out _, out ulong[] systems));
            Assert.IsEmpty(hashes);
            Assert.IsEmpty(systems);
        }
        ...
        public void L2_CountNotMatchingByteLength_IsRejected()
        {
            ...
            System.Array.Copy(blob, truncated, truncated.Length);
            Assert.IsFalse(DesyncProbePayload.TryUnpackL2(truncated, out _, out _, out _));

            byte[] padded = new byte[blob.Length + 4];
            System.Array.Copy(blob, padded, blob.Length);
            Assert.IsFalse(DesyncProbePayload.TryUnpackL2(padded, out _, out _, out _));
        }

        [Test] // a type count that would index past the blob must be refused before any allocation
        public void L2_OversizedTypeCount_IsRejected()
        {
            var blob = new byte[8];
            blob[0] = 0xFF; blob[1] = 0xFF; blob[2] = 0xFF; blob[3] = 0x7F;

            Assert.IsFalse(DesyncProbePayload.TryUnpackL2(blob, out _, out _, out _));
        }

        [Test] // participantCount that overflows int32 in the length check (… + participantCount*8) must not slip through
        ...
        [Test] // the three probe messages must survive the generated serializer, ids and all
        public void ProbeMessages_RoundTripThroughSerializer()
        {
            ...
            Assert.AreEqual(7, decodedRequest.CorrelationId);
            Assert.AreEqual(DesyncProbeLevel.Breakdown, decodedRequest.LevelEnum);
            Assert.AreEqual(120, decodedRequest.FromTick);
            Assert.AreEqual(120, decodedRequest.ToTick);
            Assert.AreEqual(3, decodedRequest.RequesterPlayerId);
            ...
            Assert.AreEqual(7, decodedResponse.CorrelationId);
            Assert.AreEqual(100, decodedResponse.BaseTick);
            Assert.AreEqual(0x1234_5678_9ABC_DEF0, decodedResponse.CmdHashAtTick);
            Assert.IsTrue(DesyncProbePayload.TryUnpackL1(decodedResponse.Payload, out int[] ticks, out long[] totals));
            Assert.AreEqual(new[] { 100, 101 }, ticks);
            Assert.AreEqual(new[] { 5L, 6L }, totals);
            ...
            Assert.AreEqual(42, decodedVerdict.DivergedTick);
            Assert.AreEqual(1, decodedVerdict.Class);
            Assert.AreEqual(2, decodedVerdict.Layer);
            Assert.AreEqual(5, decodedVerdict.TypeIdOrParticipantIdx);
            Assert.AreEqual(-1, decodedVerdict.LocalHash);
            Assert.AreEqual(99, decodedVerdict.RemoteHash);
        }
