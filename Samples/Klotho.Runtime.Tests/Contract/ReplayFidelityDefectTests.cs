...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    public sealed class ReplayFidelityDefectTests
    {
        ...
        public void PredictedTick_IsRecordedToReplay()
        {
            ...
            Assert.IsTrue(h.Engine.LastVerifiedTick >= predictTick,
                $"predictTick must be verified before Stop (LastVerifiedTick={h.Engine.LastVerifiedTick})");
            Assert.IsTrue(replay.GetCommandsForTick(predictTick).Count >= 1,
        ...
        public void LateRealCommand_OverwritesStaleEmptyFill_InReplay()
        {
            ...
            XAssert.Contains(recorded, c => c is StopCommand);
        }
        ...
        public void CorrectiveReset_TruncatesRecordingAtPreResetVerifiedTick()
        {
            ...
            Assert.IsTrue(h.Engine.IsRecording, "recording should be active before reset");
            ...
            Assert.IsFalse(h.Engine.IsRecording, "corrective reset must truncate (stop) recording");
            IReplayData replay = h.Engine.GetCurrentReplayData();
            Assert.IsNotNull(replay);
            Assert.AreEqual(preResetVerified, replay.Metadata.TotalTicks);
            Assert.IsTrue(replay.Metadata.TotalTicks < resetTick,
        ...
        [Test] // The F47 truncation was wired only into the guest receive path (HandleFullStateReceived).
        ...
        public void HostCorrectiveReset_TruncatesRecording()
        {
            ...
            Assert.IsFalse(h.Engine.IsRecording, "host corrective reset must truncate (stop) recording");
            IReplayData replay = h.Engine.GetCurrentReplayData();
            Assert.IsNotNull(replay);
            Assert.AreEqual(preResetVerified, replay.Metadata.TotalTicks);
            Assert.IsTrue(replay.Metadata.TotalTicks < resetTick,
