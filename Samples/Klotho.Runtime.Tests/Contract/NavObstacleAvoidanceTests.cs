...
namespace xpTURN.Klotho.Core.Tests
{
    ...
    public class NavObstacleAvoidanceTests
    {
        ...
        public void T1_LoadObstacles_SingleSquareRing_ComputesUnitDirConvexPrevNext()
        {
            ...
            Assert.AreEqual(4, av.DebugObstacleCount);
            var o = av.DebugObstacles;

            // prev/next wrap within the single ring
            Assert.AreEqual(1, o[0].nextIndex);
            Assert.AreEqual(3, o[0].prevIndex);
            Assert.AreEqual(0, o[3].nextIndex);
            Assert.AreEqual(2, o[3].prevIndex);
            ...
            for (int i = 0; i < 4; i++)
            {
                Assert.IsTrue(o[i].isConvex);
                Assert.AreEqual(0, o[i].polygonIndex);
            }
        }
        ...
        public void T1_LoadObstacles_MultipleRings_PrevNextWrapWithinOwnRing()
        {
            ...
            Assert.AreEqual(8, av.DebugObstacleCount);
            var o = av.DebugObstacles;

            // ring0 boundary: last vertex wraps to ring0 start (0), NOT into ring1
            Assert.AreEqual(0, o[3].nextIndex);
            Assert.AreEqual(2, o[3].prevIndex);
            Assert.AreEqual(0, o[0].polygonIndex);

            // ring1 boundary: index 4 is ring1 start; prev wraps to ring1 end (7), not ring0
            Assert.AreEqual(5, o[4].nextIndex);
            Assert.AreEqual(7, o[4].prevIndex);
            Assert.AreEqual(4, o[7].nextIndex); // ring1 last vertex wraps to ring1 start
            Assert.AreEqual(6, o[7].prevIndex);
            Assert.AreEqual(1, o[4].polygonIndex);
            Assert.AreEqual(1, o[7].polygonIndex);
        }
        ...
        public void T1_LoadObstacles_CsrSentinelOffsets_Tolerated()
        {
            ...
            Assert.AreEqual(8, av.DebugObstacleCount);
            var o = av.DebugObstacles;
            Assert.AreEqual(0, o[3].nextIndex); // ring0 wraps
            Assert.AreEqual(4, o[7].nextIndex); // ring1 wraps
        }
        ...
        public void T2_WallAheadOfAgent_DeflectsVelocity()
        {
            ...
            Assert.IsTrue(av.DebugNumObstLines > 0, "wall in front should produce an obstacle line");
            // forward (+Z) velocity must be reduced from the desired 3 (blocked by wall)
            Assert.IsTrue(result.y.ToFloat() < 3.0f - 0.05f,
        ...
        public void T2b_LongWall_CenterApproach_EndpointsOutOfRange_StillDetected()
        {
            ...
            Assert.IsTrue(av.DebugNumObstLines > 0,
        ...
        public void T2c_NoObstacles_ReturnsDesiredVelocity()
        {
            ...
            Assert.AreEqual(0, av.DebugNumObstLines);
        ...
        public void T4_Determinism_RepeatedCalls_IdenticalResult()
        {
            ...
            FPVector2 a = Run();
            FPVector2 b = Run();
            Assert.IsTrue(a == b, $"nondeterministic: {a} vs {b}"); // exact FP64 equality
        }
        ...
        public void T6_ManySegments_ObstacleLinesCappedAndAgentSlotsPreserved()
        {
            ...
            Assert.IsTrue(av.DebugNumObstLines <= FPNavAvoidance.MAX_OBST_LINES,
            ...
            Assert.IsTrue(FPNavAvoidance.MAX_ORCA_LINES - av.DebugNumObstLines >= FPNavAvoidance.MAX_NEIGHBORS);
            ...
            Assert.AreEqual(FPNavAvoidance.MAX_OBST_LINES, av.DebugSelectedObstacleCount);
        }
        ...
        public void T6b_ObstacleAndAgentLines_Coexist_AgentSlotsNotStarved()
        {
            ...
            Assert.IsTrue(av.DebugNumObstLines >= 1, "the wall must still produce an obstacle line");
            // Agent lines occupy the reserved slots on top of the obstacle lines — not starved.
            Assert.IsTrue(av.DebugOrcaLineCount > av.DebugNumObstLines,
        ...
        public void T11_ComputeNewVelocity_ObstaclePath_ZeroAllocations()
        {
            ...
            Assert.AreEqual(0, allocated / iterations);
        }
        ...
        public void T3_NeighborPushesIntoWall_ObstacleLineNeverRelaxed()
        {
            ...
            Assert.IsTrue(av.DebugInfeasibleCount >= 1, "the conflict should drive LP2 infeasible → LP3");
            Assert.IsTrue(av.DebugNumObstLines >= 1, "the wall must produce an obstacle line");
            ...
            for (int i = 0; i < av.DebugNumObstLines; i++)
            {
                ...
                Assert.IsTrue(det.ToFloat() >= -1e-3f,
            ...
            Assert.IsTrue(result.y.ToFloat() < 0.6f,
        ...
        public void T5_CornerApproach_MultiTick_NeverPenetratesBox()
        {
            ...
            Assert.IsTrue(worst < 0.05, $"agent center penetrated the box (worst depth {worst})");
            ...
            Assert.IsTrue(moved > 1.0, $"agent made no progress around the corner (moved {moved:F2} from spawn — jam?)");
        }
        ...
        public void T7_NearWallOccludesFar_AndOrderIndependent()
        {
            ...
            Assert.AreEqual(nearOnly, avNF.DebugNumObstLines);
            // order independence: registration order does not change the outcome
            Assert.AreEqual(avNF.DebugNumObstLines, avFN.DebugNumObstLines);
            Assert.IsTrue(rNF == rFN, $"registration order changed the result: {rNF} vs {rFN}");
        }
        ...
        public void T8_OverlappingWall_ObstacleLineAnchoredAtOrigin_NoReciprocal()
        {
            ...
            Assert.IsTrue(av.DebugNumObstLines >= 1);
            ...
            Assert.IsTrue(av.DebugOrcaLines[0].point == FPVector2.Zero,
        ...
        public void T10_AgentAtRadiusDistance_NoNegativeSqrtThrow()
        {
            ...
            var frame = sim.Frame;
            // no ArgumentException from FP64.Sqrt on a negative argument
            Assert.DoesNotThrow(() => av.ComputeNewVelocity(0, ref frame, entities, 1, DT));
            Assert.IsTrue(av.DebugNumObstLines >= 1);
        }
        ...
        {
            Assert.IsTrue(System.Math.Abs(expected.x.ToFloat() - actual.x.ToFloat()) < tol
