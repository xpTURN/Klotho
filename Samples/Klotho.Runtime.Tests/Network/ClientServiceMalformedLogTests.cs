...
{
    /// <summary>
    /// Client-side KWarning on malformed payloads.
    ...
    public class ClientServiceMalformedLogTests
    {
        ...
        public void ServerDrivenClientService_Malformed_LogsWarning()
        {
            ...
            Assert.IsTrue(_logger.Contains(KLogLevel.Warning, "[ServerDrivenClientService]"),
        ...
        public void SpectatorService_Malformed_LogsWarning()
        {
            ...
            Assert.IsTrue(_logger.Contains(KLogLevel.Warning, "Malformed payload"));
            Assert.IsTrue(_logger.Contains(KLogLevel.Warning, "[SpectatorService]"));
            Assert.AreEqual(0, _transport.DisconnectPeerCallCount);
        }
        ...
        public void KlothoConnection_Malformed_LogsWarning()
        {
            ...
            Assert.IsTrue(_logger.Contains(KLogLevel.Warning, "Malformed payload"));
            Assert.IsTrue(_logger.Contains(KLogLevel.Warning, "[KlothoConnection]"));
        }
        ...
        public void ServerDrivenClientService_Valid_NoWarning()
        {
            ...
            foreach (var entry in _logger.Entries)
                if (entry.Level == KLogLevel.Warning && entry.Message.Contains("Malformed payload"))
                    malformedWarnings++;
        ...
        public void KlothoConnection_AfterCompleted_Malformed_NoLog()
        {
            ...
            Assert.IsFalse(_logger.Contains(KLogLevel.Warning, "Malformed payload"),
        ...
        private static KlothoConnection ConstructKlothoConnection(INetworkTransport transport, IKLogger logger)
        {
            // KlothoConnection has a private ctor — invoke via reflection.
            var ctor = typeof(KlothoConnection).GetConstructor(
                BindingFlags.Instance | BindingFlags.NonPublic,
                binder: null,
                types: new[] { typeof(INetworkTransport), typeof(IKLogger) },
                modifiers: null);
