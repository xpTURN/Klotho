    ...
    public class TestTransport : INetworkTransport
    {
        ...
        public void DisconnectPeer(int peerId)
        {
            ...
            if (_peers.TryGetValue(peerId, out var client))
            {
                ...
                client._lastDisconnectPayload = -1;
        ...
        public void DisconnectPeer(int peerId, byte[] data)
        {
            ...
            {
                client._isConnected = false;
                client._lastDisconnectPayload = (data != null && data.Length >= 1) ? data[0] : -1;
                // Server-initiated reject disconnect carrying a reason byte → client observes RemoteDisconnect + payload.
                client.OnDisconnected?.Invoke(DisconnectReason.RemoteDisconnect);
                _peers.Remove(peerId);
                NotifyPeerDisconnected(peerId);
            }
        }
