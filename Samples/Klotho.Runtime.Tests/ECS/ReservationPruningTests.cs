...
namespace xpTURN.Klotho.ECS.Tests
{
    ...
    public class ReservationPruningTests
    {
        ...
        // NOTE (no unit test — intentional): Freeze-eq must compare the EFFECTIVE prune-set, not the raw list —
        // two rooms passing different-but-equivalent raw lists (e.g. {core} vs empty, both collapsing to null)
        // must NOT spuriously conflict. This is not unit-testable here: in test/editor/DEBUG builds a freeze
        // conflict RECOMPUTES rather than throws, so a raw-comparison bug would pass silently; the spurious-throw
        // symptom only appears in release builds (whose throw branch is compiled out of the test build).
        // Verified by reasoning + a release build rather than an [Ignore]d placeholder test (which the VS Code
        // Test Explorer surfaces as "could not be run").
    }
}
