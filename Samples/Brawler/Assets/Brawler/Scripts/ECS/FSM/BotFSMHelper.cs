...
namespace Brawler
{
    ...
    public static class BotFSMHelper
    {
                                                ...
                                                IKLogger logger = null)
        {
            ...
            logger?.KWarning($"[BotFSM] P{playerId} {context}: SnapFailed | desired=({desired.x.ToFloat():F2},{desired.z.ToFloat():F2}) self=({fallbackPos.x.ToFloat():F2},{fallbackPos.z.ToFloat():F2})");
            snapOk = false;
            return desired;
        }
